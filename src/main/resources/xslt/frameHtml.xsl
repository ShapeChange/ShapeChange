...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="/">
    ...
    <xsl:result-document format="html5" href="{$outputdir}/index.html">
      ...
      <html lang="{$lang}">
        ...
        <body>
          <div class="bannerDiv">
            <iframe frameborder="0" name="bannerFrame" scrolling="no" src="banner.html"
              title="Banner"/>
          </div>
  ...
  <xsl:template match="ApplicationSchema | Package">
    ...
    <xsl:result-document format="html5" href="{concat($outputdir,'/',$path,'/package-frame.html')}">
      ...
      <html lang="{$lang}">
        ...
        <body>
          ...
          <xsl:if
            test="$catalog/FeatureType[package[@idref = $package/@id] and type = 'Object Type']">
            <div>
              <h2 class="sidepanel">
                <xsl:value-of select="$fc.ObjectTypes"/>
              </h2>
    ...
    <xsl:result-document format="html5"
      href="{concat($outputdir,'/',$path,'/package-summary.html')}">
      <xsl:if test="$debug">
        <xsl:message>
          <xsl:text>Generating </xsl:text>
          ...
          <div id="main">
            ...
            <xsl:for-each select="$catalog/Package[parent/@idref = $package/@id]">
              ...
              <p class="indent">
                ...
                <xsl:text>: </xsl:text>
                <a
                  href="{concat($backpath,$packages[id = $subpackage/@id]/path,'/package-summary.html')}">
                  <xsl:value-of disable-output-escaping="yes" select="name"/>
                </a>
              </p>
            </xsl:for-each>
                          ...
                          <xsl:choose>
                            <xsl:when test="$noAlphabeticSortingForProperties = 'true'">
                              <xsl:for-each select="$featuretype/characterizedBy/@idref">
                                ...
                                <xsl:apply-templates mode="table_overview"
                                  select="key('modelElement', .)">
                            ...
                            <xsl:otherwise>
                              <xsl:for-each
                                select="key('modelElement', $featuretype/characterizedBy/@idref)">
  ...
  <xsl:template name="packagepath">
    ...
    <xsl:if test="$package-node/parent">
      <xsl:call-template name="packagepath">
        <xsl:with-param name="package-node"
          select="key('modelElement', $package-node/parent/@idref)"/>
      </xsl:call-template>
      <xsl:text>/</xsl:text>
    </xsl:if>
    ...
    <xsl:if test="$package">
      <tr>
        <td>
          ...
          <p class="indent">
            <a
              href="{concat($backpath,$packages[id = $package/@idref]/path,'/','package-summary.html')}">
              <xsl:value-of disable-output-escaping="yes"
