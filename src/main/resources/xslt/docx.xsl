...
<!--                <xsl:value-of disable-output-escaping="no" select="format-number(@height * 9525,'#')"/>-->
      ...
      <w:tc>
        ...
        <w:tbl>
          ...
          <xsl:call-template name="attentry">
       	  ...
       	  <xsl:variable name="val"><xsl:choose><xsl:when test="$featureAtt/voidable[.='true' or .='1']"><xsl:value-of select="$fc.true"/></xsl:when><xsl:otherwise><xsl:value-of select="$fc.false"/></xsl:otherwise></xsl:choose></xsl:variable>
  ...
  <!-- TBD: merge entrytext and attentrytext templates -->
