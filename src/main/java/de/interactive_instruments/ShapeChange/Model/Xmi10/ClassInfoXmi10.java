		...
		if (cat == Options.FEATURE || cat == Options.OBJECT
			...
			if (st != null) {
				for (Iterator<String> i = st.iterator(); i.hasNext();) {
					String sid = i.next();
					ClassInfo cix = doc.fClasses.get(sid);
					if (cix != null) {
						if ((cat == cix.category()
								&& cix.category() != Options.UNKNOWN)
							...
							if (this.model().isInSelectedSchemas(this)) {
								doc.result.addError(null, 108, name());
								doc.result.addDebug(null, 10003, name(),
										"" + cat, "!FALSE");
								doc.result.addDebug(null, 10003, name(),
										"" + cix.category(), "!TRUE");
							} else {
			...
			if (stsize > 1) {
				...
				if (this.model().isInSelectedSchemas(this)) {
					doc.result.addError(null, 109, name());
				} else {
					/*
