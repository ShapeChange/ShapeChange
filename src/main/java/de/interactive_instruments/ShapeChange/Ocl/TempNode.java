...
/**
 * <p>TempNodes are the constituents of a first level syntax tree for the 
 * supported subset of OCL. They are constructed from the syntactic structure of
 * an OCL expression alone, without referring to its context, which is given by
 * the UML class the expression belongs to and its UML environment.</p>
 * <p>Tempnode itself is abstract and comes in 10 concrete flavors, namely:</p>
 * <ul>
 ...
 * ...</i>,
 * <li>Arguments - comprising iterator variable Declarations (optionally) 
 * and operation call arguments. 
 * </ul> 
 * <p>TempNodes carry an array of SourceReferences which bundles and aggregates
 * the full scope of each TempNode. Each input line is represented by a separate
 * SourceReference.
 * 
 * @version 0.1
...
abstract class TempNode {
	...
	/**
	 * <p>The SourceReferences attached to a TempNode object are returned. They
	 * come as an array of SourceReference objects, which is ordered by line
	 * number and token serial number.</p>
	...
	/**
	 * <p>This function adds a SourceReference object to the SourceReferences 
	 * of a TempNode.</p>
	 * @param sourceRef SourceReference object to be added.
	 */
	...
	/**
	 * <p>This is a convenience function to add the SourceReference of a
	 * Token to the SourceReferences of a TempNode.</p>
	 * @param token Token, the SourceReference of which is to be added.
	 */
	void addSourceReference( Token token ) {
		SourceReference sr = token.getSourceReference();
		if( sr!=null ) addSourceReference( sr );
	}
	
	/**
	 * <p>This convenience function adds all SourceReference of the
	 * argument TempNode to the SourceReference of this TempNode.</p>
	 * @param tn TempNode the SourceReferences of which are to be added.
	 */
	void addSourceReference( TempNode tn ) {
		// Make sure all arrays are properly in place ... 
		if( tn.sourceReferences==null ) return;
		if( sourceReferences==null )
			sourceReferences = new SourceReference[0];
		// Pass to static SourceReference merger ...
		sourceReferences = SourceReference.merge(
			sourceReferences, tn.sourceReferences, true );
	}
	
	/**
	 * <p>The abstract method debugPrint outputs the content of a TempNode for
	 * the purpose of debugging this software.</p>
	 * @param stream PrintWriter onto which the debug output is to be directed.
	 */
	abstract void debugPrint( PrintWriter stream );
	
	/**
	 * <p>This defines an abstract standard method for model assignment to any
	 * of the TempNodes.
	 * @param p parser object
	 * @param model the UML model
	 * @param varctx the variable declaration context 
	 * @return generated OclNode
	 */
	abstract OclNode connectToModel( 
		OclParser p, Model model, OclNode.Declaration varctx );
	...
	static class Expression extends TempNode { 
		 ...
		 * @param type The TokenType of the introducing expression
		 * keyword such as <i>inv</i>.
		 * @param name The name of the constraint if given
		 * @param expr The TempNode representing the value of the 
		 * expression.
		 */
		...
		/**
		 * <p>This connects a TempNode.Expression to the model. Before 
		 * delegating to the standard interface connectToModel() the function
		 * checks, whether the a correct context is available.</p>
		 * @param p Parser object 
		 * @param ctx Context of the expression (class or property)
		 * @return
		 */
		OclNode.Expression connectToModelWithContext( 
			OclParser p, Info ctx ) {
			
			// Check if context is set
			if( ctx==null ) {
				MessageCollection.Message mess = 
					p.getMessageCollection().new Message( 0 );
				mess.substitute( 
					1, "Cannot connect Expression without class context." );
		...
		/**
		 * <p>This connects a TempNode.Expression to the UML model. This 
		 * includes the checks necessary to ensure that correct data types are
		 * delivered. The method also makes sure that a correct implicit 
		 * declaration for <i>self</i> is established before proceeding, 
		 * including additional environment variable declarations, which
		 * constitute an extension to this implementation of OCL.</p>
		 * <p>Note that all this requires that at least a class context is 
		 * established from the caller side.</p>
		 * @param p parser object
		 * @param model the UML model
		 * @param varctx the variable declaration context 
		 * @return The OclNode.OclExpression or null
		 */
		OclNode connectToModel(
			OclParser p, Model model, OclNode.Declaration varctx ) {
			...
			if( expressionType==Token.Type.INV ) 
				...
				MessageCollection.Message mess = 
					p.getMessageCollection().new Message( 0 );
				mess.substitute( 
					1, "Unsupported OCL expression type '" + 
					expressionType.name() + "'." );
				mess.addSourceReferences( sourceReferences );			
				return null;								
			}
			...
			if( et==OclConstraint.ConditionType.DERIVE || 
				...
				if( generalContext==null ) {
					MessageCollection.Message mess = 
						p.getMessageCollection().new Message( 0 );
					mess.substitute( 
						1, "Cannot connect init/derive expression without " +
						"Property context.");
					mess.addSourceReferences( sourceReferences );			
					return null;													
				}
			...
			OclNode.Declaration dcl =
				new OclNode.Declaration( "self", type, null, last, null, true ); 
			...
			OclNode.Expression ex =
				new OclNode.Expression( 
					expressionName, et, exp, dcl, p.environmentDeclarations ); 
			dcl.ownerNode = ex;
			for( OclNode.Declaration decl : p.environmentDeclarations )
				decl.ownerNode = ex;
			...
			if( et==OclConstraint.ConditionType.INVARIANT ) {
				if( extype.builtInType!=BuiltInType.BOOLEAN ) {
					MessageCollection.Message mess = 
						p.getMessageCollection().new Message( 41 );
					mess.substitute( 1, extype.name );
					mess.addSourceReferences( sourceReferences );			
					return null;
				} else if( isMult ) {
					MessageCollection.Message mess = 
						p.getMessageCollection().new Message( 45 );
					mess.addSourceReferences( sourceReferences );			
					return null;					
				}
			} else {
				...
				de.interactive_instruments.ShapeChange.Type ptype = 
					pi.typeInfo();
				...
				if( ! extype.isSubTypeOf(ttype) ) {
					MessageCollection.Message mess = 
						p.getMessageCollection().new Message( 42 );
					...
					MessageCollection.Message mess = 
						p.getMessageCollection().new Message( 46 );
					mess.addSourceReferences( sourceReferences );			
					return null;										
				}
			}
	...
	/**
	 * <p>Invalid objects stand for a primary item which should be present
	 * in the syntax, but isn't. It shall be treated as special constant
	 * leading to a system error, if compiled.</p>
	 */
	static class Invalid extends TempNode { 
		 ...
		 * system error. TempNode syntax trees <b>shall not</b> be connected to the
		 * model if they contain Invalid nodes.
		 * @param p parser object
		 * @param model the UML model
		 * @param varctx the variable declaration context 
		 * @return Always null.
		...
		OclNode connectToModel( 
			...
			MessageCollection.Message mess = 
				p.getMessageCollection().new Message( 0 );
			mess.substitute( 
				1, "Attempt to connect INVALID TempNode to the model." );
			mess.addSourceReferences( sourceReferences );			
			return null;
		}
	...
	/**
	 * <p>Infix objects represent all sorts of infix operations in all binding
	 * levels: implies - logical - identity - comparison - addition - 
	 * multiplication - referencing. TempNode.Infix objects carry the 
	 * operation token type and references to both operands.</p> 
	 */
	static class Infix extends TempNode {
		...
		/**
		 * <p>This constructs a TempNode.Infix out of its constituents.
		 * The SourceReferences of both operands are automatically included.</p>
		 * @param type Token type of infix operation.
		 * @param op1 TempNode reference to left operand
		 * @param op2 TempNode reference to right operand
		 */
		...
		/**
		 * <p>This implements the abstract standard method for model assignment
		 * of infix operations.</p>
		 * <p>ARROW (->) and DOT (.) are translated to OclNode.PropertyExp 
		 * objects of the proper kind (AttributeExp, OperationExp, IteratorExp)
		 * considering the classes from the UML model.</p>
		 * <p>All other Infix operations will be matched against the table
	     * of defined operations and are transformed to OclNode.OperationExp 
		...
		OclNode connectToModel( 
			...
			case DOT:
				...
				if( !( operand2 instanceof Property ) ) {
					...
					MessageCollection.Message mess = 
						p.getMessageCollection().new Message( 25 );
				...
				if( ci!=null && type!=Token.Type.ARROW ) {
					...
					result = property.connectAsUmlProperty( 
						p, model, varctx, object, false );
				} else {
					// Otherwise make attempt to find a built-in ...
					result = property.connectAsBuiltInProperty(
						p, model, varctx, object, type==Token.Type.ARROW, 
						false );
				}
				break;
			...
			default:
				...
				for( OclNode.BuiltInDescr bid : OclNode.operSymbDescriptors ) {		
					// Check name ...
					if( ! name.equals(bid.name) ) continue;
					// Check type of object ...
					OclNode.BuiltInType bidtype = bid.applType;
					if( bidtype!=OclNode.BuiltInType.ANY ) {
						if( bidtype!=bit1 ) continue;
					}
					// Check number of arguments (must be 1)...
					int bidnargs = 0;
					if( bid.arguTypes!=null ) bidnargs = bid.arguTypes.length;
					if( bidnargs!=1 ) continue;
					// Check type of argument
					if( bid.arguTypes[0]!=OclNode.BuiltInType.ANY ) {
						if( bid.arguTypes[0]!=bit2 ) continue;
					}
					// O.k., we found it. Create and return the OperationCallExp
					result = new OclNode.OperationCallExp( 
						op1, false, name, new OclNode[] { op2 }, 
						new DataType(bid.resType), MultiplicityMapping.ONE2ONE, 
						false );
					break;
				}
				if( result==null ) {
					...
					MessageCollection.Message mess = 
						p.getMessageCollection().new Message( 43 );
	...
	/**
	 * <p>Prefix objects represent the prefix operations MINUS (-) and 
	 * NOT (not). They carry the operation token type and and 
	 * reference the single operand.</p>
	 */
	static class Prefix extends TempNode {
		...
		/**
		 * <p>This constructs a TempNode.Prefix out of its constituents.
		 * The SourceReferences of the operand are automatically included.</p>
		 * @param type Token type of prefix operation (MINUS or NOT)
		 * @param op TempNode reference of operand
		 */
		...
		/**
		 * <p>This implements the abstract standard method for model assignment
		 * of prefix operations.</p>
		 * <p>Prefix operations will be matched against the table of defined 
		 * operations and are transformed to OclNode.OperationExp objects, which 
		 * contain the operand as the object.</p>
		 * @param p parser object
		 * @param model the UML model
		 * @param varctx the variable declaration context 
		...
		OclNode connectToModel( OclParser p, Model model, OclNode.Declaration varctx ) {
			...
			if( result==null ) {
				...
				MessageCollection.Message mess = 
					p.getMessageCollection().new Message( 44 );
				mess.substitute( 1, name );
	...
	/**
	 * <p>Property objects stand for all kinds of references using 
	 * identifiers. Reference can be either into the model or to
	 * OCL defined variables - this cannot be distinguished in this
	 * stage of processing.</p>
	 * <p>Property objects can also stand for references to operation
	 * invocations, in which case an additional argument list object is
	 * given in addition to the name object.</p>
	 */
	static class Property extends TempNode {
		...
		/**
		 * <p>This constructs a TempNode.Property, which represents a name 
		 ...
		 * Property object.</p>
		 * @param name Name node, may be qualified
		 * @param arguments Argument node, may be null
		 */
		...
		boolean hasSimpleName() {
			...
			if( ! ( name instanceof Identifier ) ) return false;
			Identifier id = (Identifier) name;
			if( id.name!=null ) return false;
			return true;
		}
		...
		String name() {
			...
			if( ! ( name instanceof Identifier ) ) return "(invalid)";
			Identifier id = (Identifier) name;
			return id.name();
		}
		
		/**
		 * Determine the number of arguments of a property. Properties, which
		 * do not possess an argument list (attributes/roles), return -1.
		...
		/**
		 * <p>This method connects this Property to the model in a stand-alone
		 * way. This means that there is no explicit object available on which
		 * the Property acts as a selector.</p>
		 * <p>There are various outputs this can generate:</p>
		 * <ul>
		...
		OclNode connectToModel( 
			...
			if( name==null || !(name instanceof Identifier) ) {
				...
				MessageCollection.Message mess = 
					p.getMessageCollection().new Message( 0 );
				mess.substitute( 
					1, "Property is named by non-Identifier token." );
				mess.addSourceReferences( sourceReferences );
				return null;
			}
			Identifier id = (Identifier)name;
			...
			for( dcl=varctx; dcl!=null; dcl=dcl.nextOuter ) {
				...
				if( ci!=null ) { 
					// A property on the UML model ...
					result = connectAsUmlProperty( 
						p, model, varctx, var, true );
				} else {
					// Otherwise make attempt to find a built-in ...
					result = connectAsBuiltInProperty(
						p, model, varctx, var, false, true );
				}
			...
			if( arguments!=null ) {
				MessageCollection.Message mess = 
					p.getMessageCollection().new Message( 32 );
				mess.substitute( 1, name() );
			...
			if( result==null ) {
				...
				MessageCollection.Message mess = 
					p.getMessageCollection().new Message( 34 );
				mess.substitute( 1, name() );
				mess.addSourceReferences( sourceReferences );
			}
		...
		/**
		 * <p>The method connects this Property as a selector to some given
		 ...
		 * and OCL-builtins which apply to UML defined classes.</p>
		 * @param p Parser object for accessing the diagnostics stream
		 * @param model The UML model object
		 * @param varctx Variable declaration context
		 * @param object OclNode object the property is supposed to act on
		 * @param quiet Do not generate diagnostics
		 * @return generated OclNode.PropertyCallExpr or null in case of error
		 */
		OclNode connectAsUmlProperty(
			OclParser p, Model model, OclNode.Declaration varctx, 
			OclNode object, boolean quiet ) {
			...
			if( ci==null ) {
				MessageCollection.Message mess = 
					p.getMessageCollection().new Message( 0 );
				mess.substitute( 
					1, "connectAsUmlProperty failed due to null object." );
				mess.addSourceReferences( sourceReferences );
				return null;
			}
			...
			if( !id.isSimpleName() ) {
				...
				if( c==null ) {
					...
					if( !quiet ) {
						MessageCollection.Message mess = 
							p.getMessageCollection().new Message( 30 );
						mess.substitute( 1, name() );
			...
			if( arguments==null ) {
				...
				if( !quiet ) {
					MessageCollection.Message mess = 
						p.getMessageCollection().new Message( 26 );
					mess.substitute( 1, name() );
					mess.substitute( 2, ci.name() );
				...
				if( args.declarations==null ) {
					...
					for( int i=0; i<parms.length; i++ ) {
						// Compile
						oclparms[i] = 
							parms[i].connectToModel( p, model, varctx );
						if( oclparms[i]==null ) return null;
						// Get type string
					...
					if( oi!=null )
						return new OclNode.OperationCallExp( 
							object, oi, oclparms, false );
				}
				// If there is no success with UML-defined operations it might
				// hit one of the OCL-added functions applicable to any type.
				OclNode res =
					connectAsBuiltInProperty( 
						p, model, varctx, object, false, quiet );
		...
		/**
		 * <p>This connects the Property as a selector to some given OclNode, 
		 * the selector representing some operation from the implemented subset
		 * of operations from the OCL specification.</p>
		 * @param p Parser object for accessing the diagnostics stream
		 * @param model The UML model object
		 * @param varctx Variable declaration context
		 * @param object OclNode object the property is supposed to act on
		 * @param arrow Flag: set operation selected (->)
		 * @param quiet Do not generate diagnostics
		 * @return generated OclNode.PropertyCallExpr or null in case of error
		 */
		OclNode connectAsBuiltInProperty(
			OclParser p, Model model, OclNode.Declaration varctx, 
			OclNode object, boolean arrow, boolean quiet ) {
			...
			ArrayList<OclNode.BuiltInDescr> bids = 
				new ArrayList<OclNode.BuiltInDescr>( 1 );
			int itersFound = 0;
			for( OclNode.BuiltInDescr bid : OclNode.builtInDescriptors ) {		
				...
				int bidnargs = 0;
				if( bid.arguTypes!=null ) bidnargs = bid.arguTypes.length;
				if( nargs!=bidnargs ) continue;
				// Check on set vs. object operation ...
				if( arrow!=bid.arrow ) continue; 
				// O.k., we found a candidate. Add it to the list
				bids.add( bid );
				// Count iterators
				if( bid.noOfDecls>0 ) itersFound++;
			}
			...
			if( bids.size()==0 ) {
				...
				ArrayList<OclParser.AddedOperationSignature> aolist =
					p.additionalOperations;
				// Try them
				for( OclParser.AddedOperationSignature ao : aolist ) {
					// Check against added operation signature
					if( ! name.equals(ao.name) ) continue;
					if( ao.objType!=null )
						if( ! type.isSubTypeOf(ao.objType) ) continue;
					if( ao.noOfArgs>=0 )
						if( ao.noOfArgs!=nargs ) continue;
			...
			if( !isAddedOper && bids.size()==0 ) {
				if( !quiet ) {
					MessageCollection.Message mess = 
						p.getMessageCollection().new Message( 27 );
					mess.substitute( 1, name );
					mess.substitute( 2, typename );
			...
			if( itersFound>0 ) {
				...
				if( args.declarations!=null ) {
					...
					for( TempNode tempdecl : args.declarations ) {
						...
						OclNode ocldecl = decl.connectToModelWithType( 
							p, model, varctx, type );
						if( ocldecl==null ) return null;
						OclNode.Declaration newVarctx = 
							(OclNode.Declaration)ocldecl;
			...
			for( int i=0; i<nargs; i++ ) {
				OclNode arg = args.expressions[i].connectToModel( 
					p, model, varctx );
				...
				if( bidtypes!=null ) {
					for( int i=0; i<bidtypes.length&&!matched; i++ ) {
						...
						if( !oclargs[i].isMultiple() ) {
							...
							if( bidtypes[i]==OclNode.BuiltInType.ANY ) continue;
							if( bidtypes[i]==
								oclargs[i].getDataType().builtInType ) continue;
						}
						// No: break
						matched = false;
					}
			...
			if( itersFound>0 ) {
				...
				if( ocldecls.size()>selectedbid.noOfDecls ) {
					if( !quiet ) {
						MessageCollection.Message mess = 
							p.getMessageCollection().new Message( 31 );
						mess.substitute( 1, name );
						mess.addSourceReferences( sourceReferences );
					}
					return null;	
			...
			type = isAddedOper ? 
				addedResultType : new DataType( selectedbid.resType );
			...
			if( ! isAddedOper ) {
				...
				case 2: // Type is type of object
					...
					break;
				case 3: // Type is instance type of class given as first argument
					if( oclargs.length>0 && 
						oclargs[0].dataType.builtInType==
							OclNode.BuiltInType.CLASS ) {
						type = new DataType( 
							((OclNode.ClassLiteralExp)oclargs[0]).umlClass );
					}
					break;
				case 4: // Reverse-find a UML type for the built-in if possible TODO
			...
			if( itersFound>0 ) {
				// We have an Iterator
				OclNode.Declaration [] ocldclarray = 
					new OclNode.Declaration[ocldecls.size()];
				ocldclarray = ocldecls.toArray( ocldclarray );
				callexp = new OclNode.IterationCallExp(
					object, name, ocldclarray, oclargs, type, 
					selectedbid.multMap ); 
				...
				callexp = new OclNode.OperationCallExp(
					object, arrow, name, oclargs, type, mm, false );
			}
		...
		void debugPrint( PrintWriter stream ) {
			...
			if( arguments!=null ) arguments.debugPrint( stream );
			stream.print( "]" );
		}
	}

	/**
	 * <p>Identifier objects hold names. Names (if referring to the UML model) 
	 * can be qualified by packages and classes. The qualification itself is 
	 * expressed by an Identifier node.</p> 
	 */
	static class Identifier extends TempNode {
		String name;
		TempNode qualification;
		/**
		 * <p>This constructor takes a qualification and a name to form an 
		 * Identifier object. The qualification must be an Identifier object
		 * or null. The SourceReferences of the qualification - if given - are
		 * automatically added.</p>
		 * @param qualification Identifier object used as scope for the 
		 * Identifier being created.
		 * @param name Name of the Identifier
		 * @param qualification Identifier node containing the qualification,
		 * may be null.
		 */
		...
		/**
		 * <p>Determine the name of the Identifier. Qualifications are prefixed
		 * using ::.</p>
		...
		/**
		 * <p>Find out whether the Identifier represents a simple, not-qualified
		 * name.<p>
		...
		/**
		 * <p>This method translates the qualification of an Identifier into a
		 * ClassInfo reference. A value of null is returned if there is no
		 * qualification or if it cannot be resolved.</p>
		 * @param p The parser object for accessing the diagnostics stream
		 * @param model The UML model object
		 * @param quiet Do no output diagnostics
		 * @return The ClassInfo object or null
		 */
		ClassInfo classFromQualification( 
			OclParser p, Model model, boolean quiet ) {
			if( qualification==null )
				return null;
			if( qualification instanceof Identifier ) {
				Identifier qualid = (Identifier)qualification;
				OclNode cls = 
					qualid.connectAsClassOrPackage( p, model, 1, quiet );
		...
		/**
		 * This auxiliary method is applied to an Identifier TempNode, which 
		 * is known to be a UML class or package element. Which of both is 
		 * actually expected is expressed by the argument 'level', where 1 
		 * means class and 2 or more means package.
		 * @param p The parser object for accessing the diagnostics stream
		 * @param model The UML model object
		 * @param level Level of nesting (1=class/2+=package)
		 * @param quiet Do not output diagnostics
		 * @return A ClassLiteralExp or a PackageLiteralExp or null (in case
		 * of error)
		 */
		OclNode connectAsClassOrPackage( 
			OclParser p, Model model, int level, boolean quiet ) {
			...
			if( level==1 ) {
				...
				if( qctx==null ) {
					...
					if( ci==null && !quiet ) {
						...
						MessageCollection.Message mess = 
							p.getMessageCollection().new Message( 21 );
						mess.substitute( 1, name );
						mess.addSourceReferences( sourceReferences );
					}
				} else {
					...
					if( ci==null || ci.pkg()!=pi ) {
						...
						if( !quiet ) {
							MessageCollection.Message mess = 
								p.getMessageCollection().new Message( 22 );
							mess.substitute( 1, name );
				...
				if( qctx==null ) {
					...
					if( pi==null && !quiet ) {
						...
						MessageCollection.Message mess = 
							p.getMessageCollection().new Message( 23 );
						mess.substitute( 1, name );
						mess.addSourceReferences( this.sourceReferences );
					}
				} else {
					...
					if( pi==null && !quiet ) {
						// Not found in package. Complain ...
						MessageCollection.Message mess = 
							p.getMessageCollection().new Message( 24 );
						mess.substitute( 1, name );
		...
		PackageInfo getPackageToName(LinkedList<PackageInfo> eval){
			...
			for(PackageInfo cpi : eval) {
				...
				try {
					...
					for( PackageInfo npi : npis ) evalSub.add(npi);
				} catch( Exception e ) {}
			}
			if(evalSub!=null)
	...
	/**
	 * <p>Declaration stands for declarations at various places in the OCL 
	 * syntax. It combines a simple name (the variable name) with a type (in 
	 * this reduced syntax limited to path names, which are represented by 
	 * a TempNode.Identifier) and an initializing expression. The latter two 
	 * items may be missing.</p> 
	 */
	static class Declaration extends TempNode {
		...
		/**
		 * <p>This constructor creates a variable declaration out of a simple
		 * name, a typename and an initializer. Both, typename and initializer
		 * may be null. If present, the SourceReferences of typename and
		 * initializer are automatically added.</p>  
		 * @param name String containing the name of the variable.
		 * @param typename Identifier TempNode, which stands for a type. May be
		 * null.
		 * @param initializer Arbitrary OCL expression. May be null.
		 */
		 ...
		 */
		OclNode connectToModelWithType( 
			OclParser p, Model model, OclNode.Declaration varctx, 
			OclNode.DataType type ) {
			...
			if( dcl.getDataType().builtInType==OclNode.BuiltInType.VOID ) {
				...
				boolean ok =
					dcl.getDataType().isSubTypeOf( type ) &&
					type.isSubTypeOf( dcl.getDataType() );
				if( !ok ) {
					MessageCollection.Message mess = 
						p.getMessageCollection().new Message( 39 );
		...
		OclNode connectToModel(
			...
			if( typename!=null ) {
				if( ! ( typename instanceof Identifier ) ) {
					MessageCollection.Message mess = 
						p.getMessageCollection().new Message( 0 );
					mess.substitute( 
						1, "Declaration type name is not an Identifer." );
					mess.addSourceReferences( typename.sourceReferences );
					return null;
				}
				Identifier typeid = (Identifier)typename;
				OclNode cls = 
					typeid.connectAsClassOrPackage( p, model, 1, true );
				if( cls!=null ) {
					OclNode.ClassLiteralExp cl =
						(OclNode.ClassLiteralExp) cls;
				...
				if( type==null ) {
					MessageCollection.Message mess = 
						p.getMessageCollection().new Message( 38 );
			...
			return new OclNode.Declaration( 
				name, type, init, varctx, null, false ); 
		}
	...
	/**
	 * <p>This is an auxiliary class, which stands for the current date and
	 * time when the expression is used - not when it is compiled.</p>
	 * <p>The class therefore has no properties except for a bit of better
	 * formatting when printed.</p>
	 * 
	 */
	...
	/**
	 * <p>This auxiliary class extends GregorianCalender and is employed to
	 * represent a Date literal value. Wrapping GregorianCalendar in DateTime
	 * has the mere purpose to allow overwriting the toString() method, which 
	 * as implemented in GregorianCalendar is not useful for debugging 
	 * purposes.</p>
	 *
	 */
	...
	/**
	 * <p>This auxiliary class is for setting up an OclVoid type literal.</p>
	 */
	...
	/**
	 * <p>Literal objects stand for constant values such as integers, floating
	 * point numbers, strings or dates. The class is generic an take types
	 * of any primitive data type required. Types to be used are Long, Double,
	 * String, etc.</p>
	 * @param <T> Type of Literal, must be one of Long, Double, String, 
	 * Boolean, TempNode.DateTime, TempNode.CurrentDateTime, TempNode.Nothing.
	 */
	static class Literal<T> extends TempNode {
		T value;
		/**
		 * </p>Constructs a literal node of some type T. Types to be used are
		 * Long, Double, String, Boolean, Calendar, CurrentDate.</p> 
		 * <p>Calendar stands for DateTimes. The local class CurrentDate 
		 * models the Date at the time of use.</p>
		...
		OclNode connectToModel(
			...
			mess.substitute( 
					1, "Attempt to create a literal of type '"+name+"'." );
			mess.addSourceReferences( this.sourceReferences );
			return null;
		}
	...
	/**
	 * <p>IfClause represents an if...then...else...endif construct. The three 
	 * parts are condition, ifPart and elsePart all represent arbitrary 
	 * OCL expressions.</p> 
	 */
	static class IfClause extends TempNode {
		...
		/**
		 * <p>This constructs an IfClause out of its three constituents.
		 * SourceReferences of the constituents are automatically added.</p>
		 * @param condition The condition of the if 
		 * @param ifPart The part used if the condition is true
		 * @param elsepart The part used if the condition is false
		 */
		...
		OclNode connectToModel(
			...
			if( cond_p==null ) return null;
			if( if_p==null ) return null;
			if( else_p==null ) return null;
			
			// Condition part must be of Boolean type.
			if( cond_p.getDataType().builtInType!=
				OclNode.BuiltInType.BOOLEAN ) {
				MessageCollection.Message mess = 
					p.getMessageCollection().new Message( 35 );
			...
			DataType common = 
				if_p.getDataType().commonSuperType(else_p.getDataType());
			if( common==null ) {
				MessageCollection.Message mess = 
					p.getMessageCollection().new Message( 36 );
	...
	/**
	 * <p>A LetClause stands for a let ... in ... construct. The declarations
	 * part and expression part are the constituents of the objects.</p> 
	 *
	 */
	static class LetClause extends TempNode {
		...
		/**
		 * <p>This constructor initializes a LetClause using an array of 
		 * declarations and and an expression. SourceReferences of the
		 * constituents are automatically added.</p>
		 * @param declarations Array of declarations
		 * @param expression The expression to which the declarations apply
		 */
		...
		/**
		 * <p>This connects a let-clause to the model. Declarations are checked
		 * for exhibiting a proper type and an initial value. The body 
		 * expression is compiled in the context of the declarations and a
		 * OclNode.LetExp is returned containing body and declarations.
		 * @param p Parser object for accessing the diagnostics stream
		 * @param model The UML model object
		 * @param varctx Variable declaration context
		...
		OclNode connectToModel(
			...
			// those.
			ArrayList<OclNode.Declaration> dcls =
				new ArrayList<OclNode.Declaration>();
				...
				for( TempNode declaration : declarations ) {
					...
					if( declaration==null ||
						...
						MessageCollection.Message mess = 
							p.getMessageCollection().new Message( 0 );
						mess.substitute( 
							1, "Illegal Declaration in 'let' construct." );
						if( declaration==null )
							mess.addSourceReferences( sourceReferences );
						else
					...
					OclNode ocldecl =
						declaration.connectToModel( p, model, varctx );
					if( ocldecl==null ) return null;
					OclNode.Declaration dcl =
						(OclNode.Declaration)ocldecl;
					// Check whether it exhibits a proper type
					if( dcl.getDataType().builtInType==
						OclNode.BuiltInType.VOID ) {
						MessageCollection.Message mess = 
							p.getMessageCollection().new Message( 37 );
			...
			OclNode.Declaration[] dclarray =
				new OclNode.Declaration[dcls.size()];
	...
	/**
	 * <p>The Arguments object represents an argument list in a property call 
	 * and - besides the arguments proper - may contain variable declarations,
	 * which stand for the binding variable in an iterative statement.</p>
	 */
	static class Arguments extends TempNode {
		...
		/**
		 * <p>This initializes an Arguments object given the declarations and the
		 * arguments proper. SourceReferences of the constituents are 
		 * automatically handled.</p>
		 * @param declarations VarDecl objects representing declarations
		 * @param expressions The arguments proper
		 */
		...
		/**
		 * <p>This implements the connectToModel abstract method on Arguments. 
		 * Since Arguments are fully treated in the context of Property there
		 * is no need to call this method on Arguments.</p>
		 * @param p Parser object for accessing the diagnostics stream
		 * @param model The UML model object
		 * @param varctx Variable declaration context
		 * @return Always null
		...
		OclNode connectToModel(
			...
			MessageCollection.Message mess = 
				p.getMessageCollection().new Message( 0 );
			mess.substitute( 
				1, "Attempt to connect TempNode.Arguments to the model." );
			mess.addSourceReferences( sourceReferences );			
			return null;
		}
