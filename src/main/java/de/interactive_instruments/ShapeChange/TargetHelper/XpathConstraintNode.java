...
/** 
 * <p>XpathConstraintNode and its concrete derivations stand for a 
 ...
 * <ul>
 * <li>Creation of XpathConstraintNode objects while interpreting the 
...
public abstract class XpathConstraintNode {
	...
	protected ArrayList<XpathConstraintNode> children = 
		new ArrayList<XpathConstraintNode>();
	 ...
	 * XpathConstraintNode.Error. In this case the whole tree is in 
	 * error.</p>
	 * @return Error flag
	 */
	...
	public static class Logic extends XpathConstraintNode {
		...
		/**
		 * Ctor
		 * @param xpathHelper The schema object
	...
	public static class Select extends XpathConstraintNode {
		 ...
		 */
		public Select( 
			XpathHelper xpathHelper, OclNode.Declaration vardecl ) {
			this.xpathHelper = xpathHelper;
			this.vardecl = vardecl;
		}
	...
	public static class AllInstances extends XpathConstraintNode {
		 ...
		 */
		public AllInstances( 
			XpathHelper xpathHelper, ClassInfo ci, 
			boolean negated ) {
			this.xpathHelper = xpathHelper;
			this.objectClass = ci;
		...
		public XpathFragment translate( BindingContext ctx ) {
			...
			if( ! objectClass.isAbstract() )
				classnames.add( 
					xpathHelper.getAndRegisterXmlName( objectClass ) );
	...
	public static class KindOf extends XpathConstraintNode {
		 ...
		 */
		public KindOf( 
			XpathHelper xpathHelper, boolean exact, boolean neg ) {
			this.xpathHelper = xpathHelper;
			this.exact = exact;
			negated = neg;
		}
		...
		public XpathFragment translate( BindingContext ctx ) {
			...
			if( ! argumentClass.isAbstract() )
				classnames.add( 
					xpathHelper.getAndRegisterXmlName( argumentClass ) );
	...
	public static class Matches extends XpathConstraintNode {
		...
		public XpathFragment translate( BindingContext ctx ) {
			...
			XpathHelper.ExtensionFunctionTemplate eft = 
				xpathHelper.extensionFunctions.get( "matches" );
			if( eft==null )
	...
	public static class Variable extends XpathConstraintNode {
		 ...
		 * compiled in a ISCURRENT context. Other variables
	...
	public static class Attribute extends XpathConstraintNode {
		 ...
		 */
		public Attribute( 
			XpathHelper xpathHelper, OclNode.AttributeCallExp attr, 
			boolean negated ) {
			this.xpathHelper = xpathHelper;
		 ...
		 */
		public Attribute(
				XpathHelper xpathHelper, AttrComp atc, 
				boolean negated ) {
				this.xpathHelper = xpathHelper;
		...
		public Attribute splitBefore( int at ) {
			...
			Attribute atrite = new Attribute(
				xpathHelper, attributes[at], negated );
			...
			if( at==0 ) {
				...
				Attribute atleft = new Attribute(
					xpathHelper, attributes[0], negated );
				for( int i=1; i<at; i++ )
					appendAttribute( attributes[i] );
	...
	public static class Literal extends XpathConstraintNode {
		 ...
		 */
		public Literal( 
			XpathHelper xpathHelper, OclNode.LiteralExp lit, 
			boolean neg ) {
			this.xpathHelper = xpathHelper;
			literal = lit;
			negated = neg;
	...
	public static class MessageComment extends XpathConstraintNode {
		...
		public MessageComment( 
			XpathHelper xpathHelper, String name ) {
			this.xpathHelper = xpathHelper;
			this.name = name;
		}
