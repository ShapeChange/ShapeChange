 ...
 * <li>Using {@link System.getProperty("line.separator")} for creation of new
 * lines.</li>
...
public class XMLWriter extends XMLFilterImpl {
	 ...
	 * 
	 * <p>
	 * Write to the writer provided. The writer must already be configured to
	 ...
	 * writer. The writer must already be configured to use the correct
	 * character encoding.
	 * </p>
	 * 
	 * @param xmlreader
	...
	/**
	 * Writes and emits character content reported for the element. Then writes
	 * the end tag.
	 * <p>
	 * Passes the endElement event on down the filter chain for further
	 * processing.
	 * 
	 * @param uri
	 *            The Namespace URI, or the empty string if none is available.
	...
	public void characters(char ch[], int start, int len) throws SAXException {
		...
		if (lastElementWasStart) {
			...
			/*
			 * We ignore character data between end and start element (e.g. line
			 * feeds) because we format the XML ourselves.
			 */
		}
	 ...
	 * <p>
	 * However, the event is passed on down the filter chain for further
	 * processing.
	 * 
	 * @param ch
	 *            The array of characters to write.
	...
	/**
	 * Convenience mechanism to write a comment. The comment is not passed on
	 * (no support for LexicalHandlers).
	 * 
	 * @param comment
	 * @throws SAXException
	 */
	...
	private void writeEsc(char ch[], int start, int length, boolean isAttVal)
		...
		for (int i = start; i < start + length; i++) {
			...
			case '\"':
				...
				break;
			default:
				// if (ch[i] > '\u007f') { // write("&#"); // // // write(Integer.toString(ch[i]));
																	// // //
																	// write(';');
																	// } else {
				write(ch[i]);
	...
	public void dataElement(String localName, String content, String attributeName,
			String attributeValue) throws SAXException {
