<?xml version="1.0" encoding="UTF-8"?><schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:sch="http://purl.oclc.org/dsdl/schematron" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" queryBinding="xslt2">
  ...
  <pattern>
    <rule context="ex17:TS17_FT1">
      ...
      <assert test="count((for $VAR1 in (current()/ex17:rFT1toFT2/*, (for $BYREFVAR in current()/ex17:rFT1toFT2/@xlink:href return key('idKey',substring-after($BYREFVAR,'#')))) return $VAR1)[for $x in . return some $y in for $VAR1 in ($x/ex17:rFT2toFT3/*, (for $BYREFVAR in $x/ex17:rFT2toFT3/@xlink:href return key('idKey',substring-after($BYREFVAR,'#')))) return $VAR1 satisfies $y/ex17:att = true()]) = 1">ts17_ft1_constraint2: rFT1toFT2-&gt;select(x|x.rFT2toFT3-&gt;exists(y|y.att = true))-&gt;size() = 1</assert>
    </rule>
  </pattern>
</schema>
