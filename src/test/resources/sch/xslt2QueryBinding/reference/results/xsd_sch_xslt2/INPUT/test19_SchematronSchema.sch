<?xml version="1.0" encoding="UTF-8"?><schema xmlns="http://purl.oclc.org/dsdl/schematron" xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <pattern>
    <rule context="ex19:TS19_FT">
      <assert test="substring(current()/ex19:att1, 1, 3 - 1 + 1) = 'foo'">ts19_ft_constraint1: att1.substring(1,3) = 'foo'</assert>
      <assert test="every $x in current()/ex19:att2 satisfies substring($x, string-length($x) - 3, string-length($x) - (string-length($x) - 3) + 1) = 'test'">ts19_ft_constraint2: att2-&gt;forAll(x|x.substring(x.size()-3,x.size()) = 'test')</assert>
    </rule>
  </pattern>
</schema>
