...
The name of the definition is the class name, in lower case.
...
(typically the feature type name in lower case). The column name is defined via
...
is used to encode all object properties that are not object references. In other words,
properties with simple type, enumeration or codelist type, or with a data type as value,
are encoded within a JSON object. NOTE: The JSON object does not have an additional
