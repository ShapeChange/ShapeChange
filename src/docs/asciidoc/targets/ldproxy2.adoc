...
https://docs.ldproxy.net/services/building-blocks/features_-_gml.html[ldproxy documentation] describes the 
necessary ldproxy configuration items. The ShapeChange ldproxy target supports generating 
...
properties directly owned by the type. The merge items are encoded in a way
...
aspect when <<ldproxy2_GML_output>> shall be supported by the resulting
...
in a separate table - the property is encoded (in the type definition) 
using the ldproxy keyword `concat` or `coalesce`. The former is used
  ...
  t8_mixin_b:
    ...
    - propertyMap:
        pMixB:
          ...
          constraints:
            required: true
    - schema: '#/fragments/t8_mixin_a1'
    - schema: '#/fragments/t8_mixin_a2'
  t8_mixin_c:
    type: OBJECT
...
A \<<dataType>> is not converted to a top-level provider type definition. Instead, 
...
*** the property matches _rule-ldp2-all-linkObjectAsFeatureRef_, the value type is encoded as a feature reference, the `type` is FEATURE_REF_ARRAY, and either `refUriTemplate` is added (if the value type is mapped to LINK - then the template is determined like the stringFormat-transformation of a link object href-member, see <<link_object,below>>), or a `refType` with the identifier of the value type.
...
*** the property matches _rule-ldp2-all-linkObjectAsFeatureRef_, the value type is encoded as a feature reference, the `type` is FEATURE_REF, and either `refUriTemplate` is added (if the value type is mapped to LINK - then the template is determined like the stringFormat-transformation of a link object href-member, see <<link_object,below>>), or a `refType` with the identifier of the value type.
