...
other constraints). The _coretable approach_ is a way to mitigate the level
of complexity. In this approach, the database schema only consists of two tables:
a coretable with one entry per object, and a separate references table, where
most of the object references are managed. 
...
* The table should have a separate primary key column 
(for ldproxy use, ideally integer valued).
* The table has a column with the object identifiers. The identifiers are used 
in object references. The column name is given by target parameter _coretableIdColumn_ 
...
represents the object reference, optionally followed by `/featureId` if either multiple
collections are targeted by the property value type (i.e., the value type is a supertype,
with multiple non-abstract types in the subtype hierarchy) or target parameter 
_coretableJsonFeatureRefWithAnyCollectionId_ is set to true (default value is false).
That specific source path encoding matches the JSON encoding that can be achieved using
xref:./JSON_Schema.adoc#rule-json-all-featureRefs[rule-json-all-featureRefs] with 
JSON Schema target parameter `featureRelProfiles=rel-as-key``, and potentially 
`featureRefWithAnyCollectionId=true`.
...
is 'geom').
* The table may also have a column to store the value of a high priority object reference,
for each object. Which property that reference embodies per feature type is domain 
specific. Target parameter _coretableRefColumn_ defines the column name (default value 
is 'ref'). Target parameter _coretableRefRelations_ is used to identify all relevant
schema properties by their name (the parameter has a comma-separated value, without a default). The column is useful to speed up certain queries. The properties identified
by target parameter _coretableRefRelations_ must have maximum multiplicity of 1, and
for each feature type only at most one of the properties must apply. That means that
not every feature type needs to have such a high priority property.
* The table may have additional columns, which will be ignored by ldproxy.
...
_coretableRefColumn_ (like in the coretable itself, see above).
...
NOTE: The coretable approach does not support object references whose sources are data types.
Such references may be encoded within the JSON-properties, but they cannot be represented
in the high-priority reference column of the coretable or in the references table.
