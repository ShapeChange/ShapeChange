...
|sqlDeferred |property, association | | |Defines if a foreign key constraint shall be declared 
as deferrable. |SQL target |SqlDdl target xref:../targets/SQL_DDL.adoc#Checking_Foreign_Key_Constraints[Checking Foreign Key Constraints]

|sqlEncodingRule |all | | |SQL encoding rule to apply. Default is '*'. |SQL target | 

|sqlInitialConstraintMode |property, association | | |Defines the initial mode of a foreign key constraint. |SQL target |SqlDdl target xref:../targets/SQL_DDL.adoc#Checking_Foreign_Key_Constraints[Checking Foreign Key Constraints]
