...
are set on the property: _originalSchemaName_ (with the name of the schema 
...
as value). Again, these information items can be useful in subsequent 
processing, such as when the xref:../targets/ldproxy2.adoc[ldproxy2 target] 
is set to generate configurations that support GML output.
...
property copy, with the name of property (P1) as value. That information 
