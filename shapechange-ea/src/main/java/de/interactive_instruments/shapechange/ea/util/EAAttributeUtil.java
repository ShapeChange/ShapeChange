...
public class EAAttributeUtil extends AbstractEAUtil {
     ...
     * attribute, NOT checking for duplicate tags.
     * <p>
     ...
     * 
     * @param att the attribute to which the tagged values shall be added
     * @param tv  collection of tagged values to add
	...
	if (!att.Update()) {
	    throw new EAException(createMessage(message(101), att.GetName(), att.GetLastError()));
	}
    }
	...
	if (!att.Update()) {
	    throw new EAException(createMessage(message(113), att.GetName(), att.GetLastError()));
	}
    }
	...
	if (!att.Update()) {
	    throw new EAException(createMessage(message(104), att.GetName(), att.GetLastError()));
	}
    }
	...
	if (!att.Update()) {
	    throw new EAException(createMessage(message(105), att.GetName(), length, att.GetLastError()));
	}
    }
	...
	if (!att.Update()) {
	    throw new EAException(createMessage(message(106), att.GetName(), att.GetLastError()));
	}
    }
	...
	if (!att.Update()) {
	    throw new EAException(createMessage(message(107), att.GetName(), att.GetLastError()));
	}
    }

    /**
     * Sets the given tagged value in the tagged values of the given attribute. If
     * tagged values with the same tag name already exist, they will be deleted.
     * Then the tagged value will be added.
     * 
     * @param att the attribute in which the tagged value shall be set
    ...
    /**
     * Sets the given tagged values in the given attribute. If tagged values with
     * the same name as the given ones already exist, they will be deleted. Then the
     * tagged values will be added.
     * 
     * @param att the attribute in which the tagged values shall be set
    ...
    /**
     * Sets attribute StereotypeEx on the given EA attribute.
     * 
     * @param att          tbd
     ...
     */
    public static void setEAStereotypeEx(Attribute att, String stereotypeEx) throws EAException {

	att.SetStereotypeEx(stereotypeEx);

	if (!att.Update()) {
	    throw new EAException(createMessage(message(109), att.GetName(), att.GetLastError()));
	}
    }

    /**
     * Retrieves the first tagged value with given name of the given attribute. Does
     ...
     * 
     * @param att    attribute that contains the tagged values to search
    ...
    /**
     * @param att tbd
     * @return sorted map of the tagged values (key: {tag name}; value: according
     *         EATaggedValue); can be empty but not <code>null</code>
     */
     ...
     * @throws EAException tbd
     */
    public static org.sparx.AttributeConstraint addConstraint(Attribute att, String name, String type, String text)
	    throws EAException {
    ...
    /**
     * Updates the tagged value of the given attribute. Does NOT delete the tagged
     * value.
     * <p>
     ...
     * 
     * @param att the attribute in which the tagged value shall be updated
     * @param tv  tagged value to set, must not be <code>null</code>
     * @throws EAException If updating the attribute did not succeed, this exception
     *                     contains the error message.
     */
    public static void updateTaggedValue(Attribute att, EATaggedValue tv) throws EAException {
	updateTaggedValue(att, tv.hasFQName() ? tv.getFQName() : tv.getName(), tv.getValues().getLast(),
		tv.createAsMemoField());
    }

    /**
     * Updates the tagged value with given name (which can be a fully qualified
     * name) in the tagged values of the given attribute. Does NOT delete the tagged
     * value and does NOT create a new tag.
     * <p>
     * NOTE: This method is especially useful when setting tagged values that are
     * defined by an MDG / UML Profile, since these tagged values cannot be created
     * programmatically (they are created by EA - for further details, see
     * http://sparxsystems.com/forums/smf/index.php?topic=3859.0).
     * 
     * @param att               the attribute in which the tagged value shall be
     *                          updated
     ...
     *                     contains the error message.
     */
    public static void updateTaggedValue(Attribute att, String name, String value, boolean createAsMemoField)
	    throws EAException {
    ...
    /**
     * Checks if StereotypeEx of the attribute contains one of the given
     ...
     * @return <code>true</code>, if StereotypeEx of the attribute contains one of
