...
public class ArcGISWorkspaceConstants {
    ...
    /**
     * If a feature type has the tagged value 'HasZ' set to 'true', and the feature
     * type is converted to an ArcGIS feature class (Point, Polyline, etc.), then
     * with this rule enabled the ArcGIS feature class will have the tagged value
     * 'HasZ' set to 'true' (default is 'false').
     */
    public static final String RULE_CLS_HASZ = "rule-arcgis-cls-hasZ";

    /**
     * If a feature type has the tagged value 'HasM' set to 'true', and the feature
     * type is converted to an ArcGIS feature class (Point, Polyline, etc.), then
     * with this rule enabled the ArcGIS feature class will have the tagged value
     * 'HasM' set to 'true' (default is 'false').
     */
    public static final String RULE_CLS_HASM = "rule-arcgis-cls-hasM";

    /**
     * Identifies range domains for class properties based upon the tagged values
     * 'rangeMinimum' and 'rangeMaximum'. Each boundary is inclusive. If one of the
     * tagged value is not provided, the default value for that boundary is used. If
    ...
    /**
     * If this rule is enabled, ShapeChange will use the value of the tagged value
     * 'size' (must be an integer) to populate the ‘length’ tagged value of the
     * &lt;&lt;field&gt;&gt; that will represent the property in the ArcGIS model.
     ...
     * value 'size' (must be an integer) to populate the ‘length’ tagged value of
     * the &lt;&lt;field&gt;&gt; that will represent the property in the ArcGIS
     * model. This rule has higher priority than
    ...
    /**
     * If this rule is enabled then the length of a property that has a code list or
     * enumeration as value type is computed as the maximum name length from the
     * codes/enums of the value type (if codes/enums are defined by that type). This
    ...
    /**
     * NOTE: This rule identifier is deprecated. Use {@value #RULE_ALL_PRECISION}
     * instead.
     */
    ...
    /**
     * If this rule is enabled, then the base name of a relationship class will be
     * constructed from the short names of the source and target class, combined by
     * an underscore. The short name of a class is given via the tagged value
     * specified by parameter {@value #PARAM_SHORT_NAME_BY_TAGGED_VALUE}. If no
     * short name is specified, the original class name will be used as fallback.
     * Note that the base name can be subject to additional modifications (such as
     * normalization, addition of suffix to make the name unique, and clipping in
     * case that the name exceeds the allowed length).
     */
     ...
     * determine the length of a &lt;&lt;field&gt;&gt; that represents a property
     * under {@value #RULE_PROP_LENGTH_FROM_TAGGED_VALUE}.
     */
    public static final String PARAM_NAME_OF_TV_TO_DETERMINE_FIELD_LENGTH = "nameOfTaggedValueToDetermineFieldLength";

    /**
     * Optional (defaults to 0.01) - Delta to add to / subtract from a range limit
     * in case that the lower and/or upper boundary comparison operator is not
     * inclusive.
     */
     ...
     * which the resulting ArcGIS workspace (UML) model will be created.
     */
     ...
     * when used in constructing specific names of the ArcGIS workspace. Default is
     * 'shortName'.
     */
    ...
    public static final Double DEFAULT_NUM_RANGE_MIN_LOWER_BOUNDARY = Double.valueOf(-1000000000);
    public static final Double DEFAULT_NUM_RANGE_MAX_UPPER_BOUNDARY = Double.valueOf(1000000000);
    ...
    /**
     * Setting this tagged value on a code list or enumeration indicates that the
     * codes are numeric. The tagged value contains the name of the conceptual type
     * that represents the code values best, for example 'Number' or 'Integer'. The
     * ArcGIS data type will be determined by mapping that type using the map
     * entries defined in the configuration.
     * <p>
     * NOTE: The field type determined by processing this tagged value will be
     * overridden if tagged value {@value #TV_FIELD_TYPE} is also set on the code
     * list / enumeration.
     */
