 ...
 * Germany
 */
package de.interactive_instruments.shapechange.ea.target.profiletransfer;
...
public class ProfileTransferEA implements SingleTarget, MessageSource {
     ...
     * existing profiles of a model element in the EA repository. Merging means that
     * only the profiles with names that match one of the names of profiles to be
     * transferred will be overwritten - all other profiles of the model element
     * will be kept.
     * <p>
     ...
     * profile transfer (see {@value #PARAM_PROCESS_ALL_SCHEMAS}) are deleted before
     * profiles are transferred to it (even if no corresponding model elements could
     * be found in the input model).
     * <p>
     * Applies to Rule(s): none – default behavior
     */
     ...
     * classes (and their properties) from schemas that are selected for processing.
     * If this parameter is set to <code>true</code>, profiles are transferred for
     * non-prohibited classes (and their properties) from all schemas. For details
     * on non-prohibited classes, see the explanation of input parameter
     * {@value de.interactive_instruments.shapechange.core.Options#PARAM_PROHIBIT_LOADING_CLASSES_WITH_STATUS_TV}
     * .
     * <p>
     * Applies to Rule(s): none – default behavior
     */
     ...
     * Type: String
     * <p>
     * Default Value: defaults to the value of the input parameters 'inputFile' and
     * 'repositoryFileNameOrConnectionString' (the former has higher priority than
     * the latter)
     * <p>
     * Explanation: If this parameter is set in the target configuration, it
     * provides the connection info to the EA repository in which profiles shall be
     * transferred. If the parameter is not set, the target will transfer the
     * profiles into the model that is defined in the input configuration of
     * ShapeChange. For further details on this parameter, see the explanation for
     * the input parameter with this name (but keep in mind that the target will
     * assume / requires that the connection to an EA repository is given).
     * <p>
     * Applies to Rule(s): none – default behavior
     */
     ...
     * Default Value: none
     * <p>
     * Explanation: If the target parameter 'repositoryFileNameOrConnectionString'
     * is set, and the connection requires a username and password, set the username
     * with this target parameter. NOTE: If the parameter
     * 'repositoryFileNameOrConnectionString' is not set in the target
     * configuration, the target will fully rely on the information provided in the
     * input configuration. In other words, then there is no need to set the
     * parameter 'username' in the target configuration.
     * <p>
     * Applies to Rule(s): none – default behavior
     */
     ...
     * <p>
     * Explanation: If the target parameter 'repositoryFileNameOrConnectionString'
     * is set, and the connection requires a username and password, set the password
     * with this target parameter. NOTE: If the parameter
     * 'repositoryFileNameOrConnectionString' is not set in the target
     * configuration, the target will fully rely on the information provided in the
     * input configuration. In other words, then there is no need to set the
     * parameter 'password' in the target configuration.
     * <p>
     * Applies to Rule(s): none – default behavior
     */
	...
	if (!initialised) {
	    ...
	    processAllSchemas = options.parameterAsBoolean(ProfileTransferEA.class.getName(), PARAM_PROCESS_ALL_SCHEMAS,
		    false);
	    deleteExistingProfiles = options.parameterAsBoolean(ProfileTransferEA.class.getName(),
		    PARAM_DELETE_EXISTING_PROFILES, false);

	    if (options.hasParameter(ProfileTransferEA.class.getName(), PARAM_PROFILES_TO_TRANSFER)) {
		profilesToTransfer = new TreeSet<String>(options.parameterAsStringList(
			ProfileTransferEA.class.getName(), PARAM_PROFILES_TO_TRANSFER, null, true, true));
	    }
	    ...
	    if (options.hasParameter(ProfileTransferEA.class.getName(), PARAM_REPO_CONNECTION_STRING)) {

		result.addInfo(this, 29);

		repoConnectionInfo = options.parameterAsString(ProfileTransferEA.class.getName(),
		...
		if (repoConnectionInfo == null) {
		    ...
		    username = options.parameter(ProfileTransferEA.class.getName(), PARAM_USER);
		    password = options.parameter(ProfileTransferEA.class.getName(), PARAM_PWD);
		}
		...
		if (inputModelType == null || !inputModelType.equalsIgnoreCase("EA7")) {
		    ...
		    String repoFileNameOrConnectionString = options.parameter(PARAM_REPO_CONNECTION_STRING);
		...
		if (!invalidConfiguration) {
			...
			if (!eaRepo.OpenFile(connectionString)) {
			    ...
			    result.addError(this, 16, connectionString, errormsg);
			    invalidConfiguration = true;
			}

		    } else {

			if (!eaRepo.OpenFile2(connectionString, username, password)) {
			    String errormsg = eaRepo.GetLastError();
			    result.addError(this, 17, connectionString, username, password, errormsg);
			    invalidConfiguration = true;
			}
		    }
		}
	...
	org.sparx.Collection<org.sparx.Package> eaModelPackages = eaRepo.GetModels();
    ...
    /**
     * @param eaModelPkg
     * @param parentSchemaName            name of the schema to which the parent of
     *                                    this package belongs; can be
     *                                    <code>null</code>
     * @param parentSchemaTargetNamespace target namespace of the schema to which
     *                                    the parent of this schema belongs; can be
     *                                    <code>null</code>
     */
	...
	/*
	 * Identify applicable schema name and target namespace. NOTE: Both may be null
	 * if the package is not (in) a schema.
	 */
	String schemaName;
	String schemaTargetNamespace;

	if (targetNamespace != null && (parentSchemaTargetNamespace == null
		|| !targetNamespace.equalsIgnoreCase(parentSchemaTargetNamespace))) {
	...
	if (schemaTargetNamespace != null && (processAllSchemas || !skipSchema(schemaName, schemaTargetNamespace))) {
		...
		String statusTaggedValue = EAElementUtil.taggedValue(classElmt, "status");
		...
		/*
		 * Delete the profiles tagged value of the class and its properties if so
		 * configured. NOTE: This is independent of whether or not a corresponding class
		 * can be found in the input model.
		 */
		if (deleteExistingProfiles) {
			...
			if (!conType.equalsIgnoreCase("Association") && !conType.equalsIgnoreCase("Aggregation")) {
			    continue;
			}

			/*
			 * Delete profiles tagged value only for the roles that belong to the current
			 * class.
			 */
			if (con.GetClientID() == classElementId && isNavigableEnd(con, con.GetSupplierEnd())) {
			    EAConnectorEndUtil.deleteTaggedValue(con.GetSupplierEnd(), "profiles");
			}

			if (con.GetSupplierID() == classElementId && isNavigableEnd(con, con.GetClientEnd())) {
		...
		if (hasCorrespondingInputSchema) {
			...
			result.addWarning(this, 21, className, schemaName, schemaTargetNamespace);

		    } else {

			Profiles newProfilesCi = new Profiles();
			if (!deleteExistingProfiles) {
			    String profilesTV = EAElementUtil.taggedValue(classElmt, "profiles");
				...
				try {
				    ...
				    result.addError(this, 22, className, schemaName, e.getMessage());
				}
			    }
			}
			...
			// merge profiles
			newProfilesCi.put(computeProfilesToTransfer(inputModelCi.profiles()));

			try {
			    EAElementUtil.setTaggedValue(classElmt, "profiles", newProfilesCi.toString());
			} catch (EAException e) {
			    result.addError(this, 23, className, schemaName, e.getMessage());
			}
			...
			for (Attribute att : classElmt.GetAttributes()) {
				...
				result.addWarning(this, 24, className, schemaName, schemaTargetNamespace, attName);
			    } else {

				Profiles newProfilesPi = new Profiles();
				if (!deleteExistingProfiles) {
				    String profilesTV = EAAttributeUtil.taggedValue(att, "profiles");
				    if (profilesTV != null && profilesTV.trim().length() > 0) {
					try {
					    newProfilesPi = Profiles.parse(profilesTV.trim(), false);
					} catch (MalformedProfileIdentifierException e) {
					    result.addError(this, 25, className, attName, schemaName, e.getMessage());
					}
				    }
				}

				// merge profiles
				newProfilesPi.put(computeProfilesToTransfer(inputModelPi.profiles()));

				try {
				    EAAttributeUtil.setTaggedValue(att, "profiles", newProfilesPi.toString());
				} catch (EAException e) {
				    result.addError(this, 26, className, attName, schemaName, e.getMessage());
				}
			    }
			}
			...
			for (Connector con : classElmt.GetConnectors()) {
				...
				transferProfileForAssociationRole(con.GetSupplierEnd(), inputModelCi, schemaName,
					schemaTargetNamespace);
			    }

			    if (con.GetSupplierID() == classElementId && isNavigableEnd(con, con.GetClientEnd())) {

				transferProfileForAssociationRole(con.GetClientEnd(), inputModelCi, schemaName,
					schemaTargetNamespace);
			    }
			}
		    }
	...
	org.sparx.Collection<org.sparx.Package> subPackages = eaModelPkg.GetPackages();
    ...
    private void transferProfileForAssociationRole(ConnectorEnd end, ClassInfo inputModelCi, String schemaName,
	    String schemaTargetNamespace) {
	...
	if (inputModelPi == null) {
	    result.addWarning(this, 24, inputModelCi.name(), schemaName, schemaTargetNamespace, roleName);
	} else {

	    Profiles newProfilesPi = new Profiles();
	    if (!deleteExistingProfiles) {
		String profilesTV = EAConnectorEndUtil.taggedValue(end, "profiles");
		if (profilesTV != null && profilesTV.trim().length() > 0) {
		    try {
			newProfilesPi = Profiles.parse(profilesTV.trim(), false);
		    } catch (MalformedProfileIdentifierException e) {
			result.addError(this, 27, inputModelCi.name(), roleName, schemaName, e.getMessage());
		    }
		}
	    }
		...
		result.addError(this, 28, inputModelCi.name(), roleName, schemaName, e.getMessage());
	    }
	}
    }
	...
	if (profilesToTransfer == null) {
	    ...
	    ptt = new Profiles(profilesOfModelElement.getProfiles(profilesToTransfer));
	}
	...
	if (appSchemaNameRegex != null && appSchemaNameRegex != null && appSchemaNameRegex.length() > 0
	...
	if (appSchemaNamespaceRegex != null && appSchemaNamespaceRegex.length() > 0
    ...
    /**
     * @param pkgName can be <code>null</code>
     * @param pkgElmt can be <code>null</code>
     * @return target namespace of the package (from tagged value 'targetNamespace'
     *         or 'xmlNamespace', or from explicit package configuration); can be
     *         <code>null</code> if no target namespace was found
     */
	...
	if (pkgElmt != null) {
	    ...
	    org.sparx.Collection<org.sparx.TaggedValue> tvs = pkgElmt.GetTaggedValues();
		...
		for (org.sparx.TaggedValue tv : tvs) {
			...
			targetNamespace = tv.GetValue();
			/*
			 * do not break here, since we look at the remaining tagged values to see if one
			 * is a 'targetNamespace', which would be preferred
			 */
		    }
		}
	    }
