...
//	r.addRule("rule-modeldiff-");
	...
	return switch (mnr) {
	case 0 -> "Context: class DiffTarget";
	case 1 -> "";
	case 2 -> "XML Schema document with name '$1$' could not be created, invalid filename.";
	case 3 -> "Output file '$1$' already exists in directory '$2$'. Attempting to delete it...";
	...
	case 107 -> "At least one of the required parameters " + DiffTargetConstants.PARAM_REFERENCE_MODEL_TYPE + " and "
	...
	case 109 -> "START reading the reference model. Note that following log messages - until END of reading is indicated - are generated by the model loading process. Warnings and errors are related to the reference model.";
	case 110 -> "END reading the reference model.";

	default -> "(" + DiffTarget.class.getName() + ") Unknown message with number: " + mnr;
	};
    }
}
