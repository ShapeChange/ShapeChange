...
public class TargetXmlSchemaConfiguration extends TargetConfiguration {
    ...
    /**
     * map with key: propertyName#schemaName (the property name may be qualified,
     * i.e. ClassA::propertyB; the schemaName can be empty if it is undefined);
     * value: the XsdPropertyMapEntry
     */
     ...
     * @param mapEntries                    Will be ignored, so can be
     *                                      <code>null</code>.
     * @param xsdMapEntries                 The xsd map entries for the target, can
     *                                      be <code>null</code>
     * @param xsdPropertyMapEntries         tbd
     * @param xmlNamespaces                 Xml namespaces defined for this target,
     ...
     */
    public TargetXmlSchemaConfiguration(String className, ProcessMode processMode, Map<String, String> parameters,
	    Map<String, ProcessRuleSet> ruleSets, List<ProcessMapEntry> mapEntries, List<XsdMapEntry> xsdMapEntries,
	    Map<String, List<XsdPropertyMapEntry>> xsdPropertyMapEntries, List<XmlNamespace> xmlNamespaces,
	    SortedSet<String> inputIds, Element advancedProcessConfigurations, List<String> validatorIds) {
     ...
     *         qualified, i.e. ClassA::propertyB; the schemaName can be empty if it
     *         is undefined); value: the XsdPropertyMapEntry
     */
	...
	if (this.xmlNamespaces == null || xmlNamespaces.isEmpty()) {
		...
		sb.append("\t\txml namespace (" + xmlns.getNsabr() + "|" + xmlns.getNs() + "|" + xmlns.getLocation()
			+ "|");
	...
	if (this.xsdMapEntries == null || xsdMapEntries.isEmpty()) {
		...
		if (mapEntry.getEncodingRules() == null || mapEntry.getEncodingRules().isEmpty()) {
	...
	if (this.xsdPropertyMapEntries == null || xsdPropertyMapEntries.isEmpty()) {
	    sb.append("none\r\n");
	} else {
	    sb.append("\r\n");
	    for (XsdPropertyMapEntry mapEntry : this.xsdPropertyMapEntries.values()) {
     ...
     * property and has a (non-empty) target element, then the value of that target
     * element - which is a QName like string - is returned. Otherwise, the QName as
     * defined by
     * {@link de.interactive_instruments.shapechange.core.model.PropertyInfo#qname()} is
     * returned.
     * 
     * @param pi tbd
     * @return the QName of the given property, cannot be <code>null</code> target
     *         element (QName like string) of the property map entry that applies to
     *         the given property, if one is defined in the configuration and if it
     *         is not blank, else <code>null</code>
     */
	...
	if (schemaOfInClass == null) {
	    /*
	     * Can happen if the class is owned by an external package that is not a schema.
	     * Then pi could be a reverse property.
	     */
	    return null;
	} else {
	    return getPropertyMapEntry(pi.inClass().name() + "::" + pi.name(), schemaOfInClass.name());
	}
    }
     ...
     * map entry. The look-up of the XsdPropertyMapEntry that applies to P is
	...
	if (StringUtils.isBlank(propertyNameScopedToClass) || StringUtils.isBlank(schemaName)) {
	    return null;
	}

	// do we have an ideal match?
	if (xsdPropertyMapEntries.containsKey(propertyNameScopedToClass + "#" + schemaName)) {
	    return xsdPropertyMapEntries.get(propertyNameScopedToClass + "#" + schemaName);
	}
	...
	if (xsdPropertyMapEntries.containsKey(propertyNameScopedToClass + "#")) {
