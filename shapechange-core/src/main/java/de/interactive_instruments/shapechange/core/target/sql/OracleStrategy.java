...
import de.interactive_instruments.shapechange.core.target.sql.expressions.EqualsExpression;
import de.interactive_instruments.shapechange.core.target.sql.expressions.Expression;
import de.interactive_instruments.shapechange.core.target.sql.expressions.LongValueExpression;
...
import de.interactive_instruments.shapechange.core.target.sql.expressions.ToCharExpression;
...
public class OracleStrategy implements DatabaseStrategy, MessageSource {
	...
	public String message(int mnr) {
		return switch (mnr) {
		case 0 -> "Context: class OracleStrategy";
		case 3 -> "Invalid map entry for type#rule '$1$': no value is provided for the characteristic '$2$' of parameter '$3$'.";
		case 4 -> "Invalid map entry for type#rule '$1$': value provided for characteristic '$2$' of parameter '$3$' is invalid. Check that the value matches the regular expression: $4$.";
		default -> "(" + OracleStrategy.class.getName()
