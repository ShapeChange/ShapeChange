 ...
 * a subset of OCL). For the definition of Token objects see class Token.
 * </p>
 *
 * @version 0.1
...
public class Lexer {
    ...
    /**
     * The following pattern controls recognition of all OCL tokens except reserved
     * words.
     */
    private static String scanRegexJava6 = "\\s*(" + "(--.*)|" + // Line Comment
	    "(/\\*\\s*)|" + // Start of a Paragraph Comment
	    "([a-zA-Z_]\\w*)|" + // Identifier
	    "((?:\\d+\\.\\d*|\\d*\\.\\d+)(?:[eE][+-]?\\d+)?|\\d+[eE][+-]?\\d+)|" + "(\\d+)|" + // Last: Floating Point,
											       // this: Integer
	    "('(?:[^']|'')*')|" + // String Literal '...''...'
	    "\\(|\\)|" + "\\[|\\]|" + "\\{|\\}|" + "=|<>|<=|>=|<|>|" + "->|\\." + "\\+|-|\\*|/|" + "\\||,|;|::|:|"
	    + "\\S" + // Illegal material
	    ")\\s*";
    private static String scanRegexCommentJava6 = "(" + // Activate Unicode support
	    "(.*\\*/)|" + // Anything up to the end of the comment
	    "(.*/\\*)|" + // Anything up to the start of a nested one
	    "(.*)" + // Anything at all
	    ")";
    private static String scanRegexJava7 = "(?U)\\s*(" + // Activate Unicode support
	    "(--.*)|" + // Line Comment
	    "(/\\*\\s*)|" + // Start of a Paragraph Comment
	    ...
	    + "\\S" + // Illegal material
	    ")\\s*";
    private static String scanRegexCommentJava7 = "(?U)(" + // Activate Unicode support
	    "(.*\\*/)|" + // Anything up to the end of the comment
	    "(.*/\\*)|" + // Anything up to the start of a nested one
	    "(.*)" + // Anything at all
	    ")";
    ...
    private static Pattern scanPattern = (major == '1' && minor == '6'
	    ? Pattern.compile(scanRegexJava6, Pattern.MULTILINE)
	    : Pattern.compile(scanRegexJava7, Pattern.MULTILINE));
    private static Pattern scanPatternComment = (major == '1' && minor == '6'
     ...
     * from. It is important that the input is delivered by the Readable in single
     * character buffers reflecting lines of the original input. The Token stream
     * returned from the Lexer will refer to line numbers and column on these, which
     * are subsequently used in the parser to generate appropriate error messages.
     * </p>
     * 
    ...
    /**
     * <p>
     * You fetch Tokens from the Lexer object one by one by means of the fetchToken
     * function. See the definition of the Token class to understand the nature of
     * Tokens. The last Token conceptually received is an Operator token of Type
     * END_OF_TEXT. All further invocations of fetchToken will result in this kind
     * of Token object.
     * </p>
     * <p>
     * If there are "unfetched" Tokens, these will be returned prior to the ones
     * recognized from the input stream. Unfetched Tokens come in a
	...
	do {
	    // Which pattern?
	    Pattern pattern = commentStack == 0 ? scanPattern : scanPatternComment;
		...
		if (commentStack > 0) {
			...
			if (--commentStack == 0) {
			    token = new Token.Comment(serialNumber++, comment + line);
			    comment = "";
			} else {
			    comment += line;
			    continue;
			...
			token = new Token.Number(serialNumber++, line, false);
		    else if (match.end(6) - match.start(6) > 0)
			// So this must be an Integer.
			token = new Token.Number(serialNumber++, line, true);
		    else if (match.end(7) - match.start(7) > 0) {
			// This covers String literals '...'
			line = line.substring(1, line.length() - 1);
			token = new Token.Text(serialNumber++, doubleQuotes.matcher(line).replaceAll("'"));
		    } else
			// Must be illegal
			token = new Token.Illegal(serialNumber++, line);
		}
		// Return the Token determined
		token.setSourceReference(lineno, (short) (start /*-baseOffset*/), (short) (end /*-baseOffset*/));
		return token;
	    }
	...
	eotToken = new Token.Operator(serialNumber, Token.Type.END_OF_TEXT);
	eotToken.setSourceReference(lineno, (short) (end + 1/*-baseOffset*/), (short) (end + 1/*-baseOffset*/));
	return eotToken;
    }

    /**
     * <p>
     * Tokens fetched in excess can be pushed back to the Lexer object by means of
     * the unfetchToken function. An arbitrary number of tokens can be unfetched.
     * </p>
     * <p>
     * Warning: Generally, no check can be done, whether the Tokens pushed back are
     * the same as previously delivered. Do not regard this to be a feature. If a
     * Checkpoint is active, however, recording of delivered Tokens takes place. In
     * this case an <b>assert</b> makes indeed sure that Tokens are unfetched
	...
	assert checkpointCount <= 0 || (!recordingStack.isEmpty() && recordingStack.peek() == token)
    ...
    /**
     * A Checkpoint encapsulates a state of a Lexer object, which you can capture
     * and restore by means of the captureState and restoreState methods.
     */
     ...
     * encapsulated in a Checkpoint object. You can later restore to this former
     * state by means of the restoreState() method. If you do not restore you need
     * to release the Checkpoint by calling releaseState().
     * </p>
     * 
    ...
    /**
     * <p>
     * The restoreState method restores the state of Token delivery of the Lexer to
     * a former state once captured by means of the method captureState().
     * </p>
     * 
