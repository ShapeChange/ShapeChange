...
[[additionalTaggedValuesInTemporaryXml]]
==== additionalTaggedValuesInTemporaryXml
...
+++Explanation+++: A list of additional tagged values that 
shall be encoded in the temporary XML (that is the basis for
conversion via XSLT). Useful if an XSLT script needs to 
access the additional tagged values (e.g. for sorting, or for
inserting specific content), but the tags shall not explicitly
be listed in the output as tagged values.
...
[[miscellaneousContentDirectoryPath]]
==== miscellaneousContentDirectoryPath
...
+++Explanation+++: The path to a directory with miscellaneous
files and directories, that shall be copied to the output directory which
contains the resulting feature catalogue. 

NOTE: This parameter currently only applies to the FRAMEHTML output format.
...
NOTE: Since 4.1.0, parameter <<additionalTaggedValuesInTemporaryXml>> 
can be used to encode further tagged values (in addition to those 
defined by input parameter _representTaggedValues_) in the temporary
XML file generated by the target. XSLT scripts can then make use of the 
