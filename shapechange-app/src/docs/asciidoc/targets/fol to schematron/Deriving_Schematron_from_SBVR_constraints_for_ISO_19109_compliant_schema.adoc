...
in SBVR for an [underline]#ISO 19109 compliant application schema# consists 
of three steps, only two of which are required:

. Loading the conceptual schemas (required)
. Loading the constraints from an excel file and parsing them to First Order Logic constraints/expressions (optional)
...
being post-processed. In that case, the transformation to load them
from an Excel file and parse them as part of transformation post-processing
...
Loader] page
.. NOTE: Parsing the rules to First Order Logic constraints/expressions and
validating them is common transformer postprocessing
functionality (for further details, see
xref:../../transformations/Common_Transformer_Functionality.adoc#Parsing_and_Validating_Constraints[here]).
