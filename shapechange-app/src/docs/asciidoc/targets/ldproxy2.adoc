...
Stored query files are created, if according `LdproxyStoredQuery` elements are
...
The structure of the `LdproxyStoredQuery` element is defined in the 
https://shapechange.net/resources/schema/ShapeChangeConfiguration.xsd[ShapeChange Configuration XML Schema]. It has the following child elements:
...
** PropertyEqualTo: The stored query shall retrieve objects based upon an equality check 
of a parameter against a particular feature property.
