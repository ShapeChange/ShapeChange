...
// The updated ldproxy target uses platform identifier 'ldp2'. The target is the next iteration for generating ldproxy configuration files. The first iteration, with platform identifier 'ldp', has been removed.
...
Note that _label_ and _description_ for a type definition are not encoded 
in the fragment definition of the type (because that information would be redundant)
and if fragments are enabled, then _label_ and _description_ for properties are
only encoded in fragment definitions, not in the type definitions (exceptions are made
for special cases, such as link objects).
...
The target creates a feature provider configuration (in folder data/entities/providers) as well as a single service configuration file (in folder data/entities/services). The names of these files are given by the name of the xref:../get started/The_element_input.adoc#mainAppSchema[main schema], in all lower case, and non-word characters replaced by '\_'. The resulting name is also used as the id within these files. However, if target parameter _apiId_ is set, then its value is used as file name and id. The encoding details for each of the two configuration files are documented in sections <<ldproxy2_schemaconversionrules_schemapackage_providerconfig>> and <<ldproxy2_schemaconversionrules_schemapackage_serviceconfig>>.

NOTE: If a single API (feature provider and service configuration) shall be created for each schema, set the xref:../targets/Output_Targets.adoc#Selecting_the_Schemas_to_Process[schema selection parameter] appropriately. If multiple schemas need to be processed, and each shall result in a different API, then either run ShapeChange multiple times (updating the schema selection parameters each time) or define multiple ldproxy targets (each selecting a different schema for processing).

It is also possible to create a tile provider configuration 
(in folder data/entities/providers) - 
see <<ldproxy2_schemaconversionrules_schemapackage_tileproviderconfig>>. 
The name of this file is the name of the feature provider configuration file,
with name suffix "-tiles". The id is created in a similar fashion.
...
The following information items are added to the feature provider configuration file:
...
** NOTE: (since v4.1.0) If target parameter _srid_ is not configured, then nativeCrs is not created.
...
* labelTemplate: value of target parameter _providerConfigLabelTemplate_; if the parameter is not set (to a non-empty string), the 'labelTemplate' is not created
...
[[ldproxy2_schemaconversionrules_schemapackage_tileproviderconfig]]
==== Tile Provider Configuration

(since v4.1.0)

If target parameter _enableTiles_ is set to 'true',
a tile provider configuration is generated, with the following information items:

// source: https://docs.ldproxy.net/providers/tile/ and https://docs.ldproxy.net/providers/tile/10-features.html

* id: the provider name (derived from the main schema, as defined in section <<ldproxy2_schemaconversionrules_schemapackage>>), with suffix "-tiles"
* enabled: true
* providerType: TILE
* providerSubType: FEATURES
* tilesetDefaults:
** sparse: value of optional target parameter _tilesetDefaultSparse_ (boolean value; default: false)
** center:
*** lon: value of optional target parameter _tilesetDefaultCenterLon_ (double value; default: 0.0)
*** lat: value of optional target parameter _tilesetDefaultCenterLat_ (double value; default: 0.0)
** levels:
*** WebMercatorQuad:
**** min: value of optional target parameter _tilesetDefaultMinLevelWebMercatorQuad_ (integer value >= 0; default: 0)
**** max: value of optional target parameter _tilesetDefaultMaxLevelWebMercatorQuad_ (integer value > min; default: 24)
**** default: value of optional target parameter _tilesetDefaultDefaultLevelWebMercatorQuad_ (integer value; default: min value)
* tilesets:
** The "all" tileset will always be created.
** For each collection from the feature provider configuration, where a 
  primary geometry is defined, an according tileset item is added (with just the id
  of the collection).
* caches:
** Two cache definitions will be encoded, dynamic seeded and dynamic unseeded.
** dynamic seeded cache:
*** type: DYNAMIC
*** storage: PER_JOB
*** seeded: true
*** levels:
**** WebMercatorQuad:
***** min: _min_ value from tileset defaults
***** max: value of optional target parameter _dynamicTilesCacheSeededMaxLevelWebMercatorQuad_ (integer value > min; default: max(10, tileset defaults min))
** dynamic unseeded cache:
*** type: DYNAMIC
*** storage: PER_JOB
*** seeded: false
*** levels:
**** WebMercatorQuad:
***** min: 1 + _max_ value from dynamic seeded cache
***** max: _max_ value from tileset defaults

By setting target parameter __enableTiles__ to 'true', 
ShapeChange will add ldproxy building block 
https://docs.ldproxy.net/de/services/building-blocks/tiles.html[Tiles]
...
[[ldproxy2_building_block_codelists]]
===== Codelists

ldproxy supports the publication of configured code lists. The documentation of 
ldproxy building block Codelists describes the relevant configuration items.
By setting target parameter __enableCodelists__ to 'true', 
ShapeChange will add ldproxy building block 
https://docs.ldproxy.net/de/services/building-blocks/codelists.html[Codelists]
to the service configuration as enabled.


[[ldproxy2_building_block_crs]]
===== Crs

(since v4.0.0)

By setting target parameter _enableCrs_ to 'true', 
ShapeChange will add ldproxy building block 
https://docs.ldproxy.net/de/services/building-blocks/crs.html[CRS]
to the service configuration as enabled. The Web API is then
configured to support additional coordinate reference systems, and
thus the provision of data in different CRSs.

If the building block is enabled, ShapeChange will automatically add
the native CRS (defined by target parameters _srid_ and _forceAxisOrder_)
to the `additionalCrs` member of the building block. Using target parameter
_additionalCrs_, more CRS definitions can be provided for the 
`additionalCrs` member. See the documentation of the member in the official
ldproxy documentation of the building block. The value of the target 
parameter is a comma-separated list of CRS identifiers. A CRS identifier
is either a (numeric) EPSG code, or a combination (using '|' as separator) of
an EPSG code and one of the values for `forceAxisOrder` supported by ldproxy.
If the CRS identifier only is the EPSG code then `forceAxisOrder=NONE` applies.
...
----------
<Target class="de.interactive_instruments.shapechange.core.target.ldproxy2.Ldproxy2Target" mode="enabled" inputs="TRF_END">
...
is enabled by default for an ldproxy instance. By setting 
target parameter __enableFeaturesGeoJson__ to 'false', 
ShapeChange will add the building block to the service configuration as disabled.
...
//////
* If target parameter _jsonFgCoordRefSys_ is set (to a boolean value; 
no default is defined for this target),
a `coordRefSys` member will be added to the general Features JSON-FG building block 
in the service configuration.
//////
* If target parameter _jsonFgFeatureType_ is set (see description of
configuration option `featureType` in the ldproxy documentation of building 
block Features JSON-FG),
a `featureType` member will be added to the general Features JSON-FG building block 
in the service configuration.
//////
* If target parameter _jsonFgIncludeInGeoJson_ is set (see description of
configuration option `includeInGeoJson` in the ldproxy documentation of building 
block Features JSON-FG),
an `includeInGeoJson` member will be added to the general Features JSON-FG building block 
in the service configuration.
//////
* Additional configuration items for this building block can be added
through the <<ldproxy2_schemaconversionrules_specificrules_genericvaluetypes>>.
...
[[ldproxy2_building_block_features_result_type]]
===== Features - Result Type

(since v4.1.0)

ldproxy building block 
https://docs.ldproxy.net/services/building-blocks/features_-_result_type.html[Features - Result Type]
adds a query parameter to return only the number of features matched by a request.
By setting target parameter __enableFeaturesResultType__ to 'true', 
ShapeChange will add the building block to the service configuration as enabled.


[[ldproxy2_building_block_filter]]
===== Filter

(since v4.0.0)

ldproxy building block 
https://docs.ldproxy.net/de/services/building-blocks/filter.html[Filter]
supports feature filtering with CQL2.
By setting target parameter __enableFilter__ to 'true', 
ShapeChange will add the building block to the service configuration as enabled.


[[ldproxy2_building_block_projections]]
===== Projections

(since v4.1.0)

ldproxy building block 
https://docs.ldproxy.net/services/building-blocks/projections.html[Projections]
supports selecting the feature properties included in the feature response.
By setting target parameter __enableProjections__ to 'true', 
ShapeChange will add the building block to the service configuration as enabled.
...
the target checks which, if any, of the (directly owned or inherited) 
properties have a name - current or original - equal 
...
** does not match _rule-ldp2-all-linkObjectAsFeatureRef_, then link object encoding is assumed, and `.title` is added to the role name (meaning that queries will be performed on the `title`-value of the link object that is encoded for an association role). Note that link objects always have a `title`-property, with either the id of the linked object, or its title.
** otherwise, (_rule-ldp2-all-linkObjectAsFeatureRef_ does match), the queryable will be the feature id.
...
Since v4.0.0, it is also possible to tag properties that shall be queryables with 
tag 'ldpQueryable' (value must be equal to, ignoring case, 'true'). This supports marking
properties of complex data types as queryables, which is not possible using 
target parameter `queryables`. Of course, setting the tag also works for other kinds of properties (i.e.,
attributes and association roles of a class).
...
[[ldproxy2_building_block_sorting]]
===== Sorting

(since v4.1.0)

ldproxy building block 
https://docs.ldproxy.net/services/building-blocks/sorting.html[Sorting]
supports sorting features in a response.
By setting target parameter __enableSorting__ to 'true', 
ShapeChange will add the building block to the service configuration as enabled.

Additional parameters can be set, to define the sorting behavior.
They correspond to the configuration parameters of the building block.
Therefore, please visit the documentation of the building block for further details.

* Parameter _sortingIncluded_ (comma-separated list of strings, default: _none_): corresponds to the _included_ parameter of the Sorting building block
* Parameter _sortingExcluded_ (comma-separated list of strings, default: _none_): corresponds to the _excluded_ parameter of the Sorting building block
* Parameter _sortingPathSeparator_ (string, default: _none_): corresponds to the _pathSeparator_ parameter of the Sorting building block
...
** Relevant map entry parameter(s): 
*** _initialValueEncoding_ (optional, currently only used for mapping of the conceptual type 'Boolean')
*** _measure_ (optional)
* GEOMETRY
...
** Parameter _measure_ (since v4.1.0): Indicates that the map entry contains a mapping for a
...
The ShapeChange configuration and results of that test are available https://github.com/ShapeChange/ShapeChange/tree/master/shapechange-app/src/integrationtests/ldproxy2/fragmentsConcatCoalesce[here], 
while the model is available
https://github.com/ShapeChange/ShapeChange/tree/master/shapechange-app/src/integrationtests/sql/sqlEncodingInfos[here].
...
definitions are encoded. For further details, see section <<ldproxy2_schemaconversionrules_schemapackage_providerconfig>>. Label and
description are not encoded in the fragment definition of the class.
...
If a type with identity has one or more geometry properties, one of them 
should be designated as primary geometry in order for ldproxy to a) use 
this geometry for bbox-queries, and to b) encode this property within 
the GeoJSON _geometry_ or the JSON-FG _place_ member. 

NOTE: Relevant geometry properties must directly be defined by the type 
with identity (or one of its supertypes). In other words, the target does 
not look at geometry properties in inlined (data) types when searching 
for the primary geometry.

If a type has one or more geometry properties, and none is explicitly 
assigned the role of primary geometry, ldproxy automatically uses the 
first geometry property as primary geometry (first in the list of 
properties in the feature provider definition of the type).

The property that contains the primary geometry is identified by setting 
tagged value _defaultGeometry_ on the property to the value `true`. 
If multiple such properties exist (e.g. through inheritance), none of them 
is encoded by ShapeChange as primary geometry (because no informed choice 
can be made) and an error will be logged.

The primary geometry property is encoded with `role: PRIMARY_GEOMETRY`.

NOTE: If the primary geometry property has a maximum multiplicity greater 
than 1, then ShapeChange will log a warning and assume a maximum multiplicity 
of exactly 1.

NOTE: Additional geometry properties are encoded like all other UML properties.

NOTE: The ldproxy target expects all geometry properties to have a maximum 
multiplicity of exactly 1. That is because ldproxy supports multi-geometry 
types (e.g. MULTI_POINT and GEOMETRY_COLLECTION), which are typically used 
in implementations that rely upon the 
https://www.ogc.org/publications/standard/sfa/[Simple Feature Access standard] 
(such as spatial databases which typically implement the 
https://www.ogc.org/publications/standard/sfs/[Simple Feature Access - Part 2: SQL Option]), 
but does not define a transformation of a geometry property with max 
multiplicity greater than 1 to such a multi-geometry type. An according 
model transformation could be defined and implemented in ShapeChange as 
a future enhancement. In any case: If the ldproxy target encounters a 
geometry property with maximum multiplicity greater than 1, it logs an 
error and continues processing, assuming a maximum multiplicity of exactly 1.
...
If a type with identity has one or more temporal properties, i.e. properties 
whose type is mapped to DATE or DATETIME, then the ldproxy target looks for 
tagged values _defaultInstant_, _defaultIntervalStart_, and 
_defaultIntervalEnd_ with value `true` on these properties, in order to 
determine whether one of them defines the temporal extent of the type.

* Use of _defaultInstant_ and _defaultIntervalStart_ / _defaultIntervalEnd_ 
are mutually exclusive. If _defaultInstant_ is `true` on one of these properties, 
then the other tags must not be `true`, and vice-versa. An error will be 
logged if that is not the case.
* If _defaultInstant_ is `true` on one property, then the according property 
will be encoded with `role: PRIMARY_INSTANT`. If more than one such property was 
found, an error will be logged.
* If _defaultIntervalStart_ is `true` on one property, then the according 
property will be encoded with `role: PRIMARY_INTERVAL_START`. If more than one 
such property was found, an error will be logged.
* If _defaultIntervalEnd_ is `true` on one property, then the according property 
will be encoded with `role: PRIMARY_INTERVAL_END`. If more than one such property 
was found, an error will be logged.

WARNING: If none of the temporal properties of the type with identity is 
marked as PRIMARY_INSTANT, PRIMARY_INTERVAL_START, or PRIMARY_INTERVAL_END, 
then ldproxy will automatically choose the first temporal property 
(following the order in which the properties are encoded in the feature provider 
type definition) as PRIMARY_INSTANT. If this is not desirable for a given type, 
then define which of the temporal properties should be used as primary 
instant or interval start / end.

NOTE: The search for temporal properties that define primary instant / interval 
is performed on top-level properties only. I.e., properties of inlined (data) 
types are not considered by this search.

NOTE: The ldproxy target expects all default temporal properties to have a 
maximum multiplicity of exactly 1. If a default temporal property with 
maximum multiplicity greater than 1 is encountered, the target logs an 
error and continues processing, assuming a maximum multiplicity of exactly 1.

NOTE: A property with value type TM_Period cannot directly be used for the 
definition of the temporal extent of a feature, at least not with this 
ShapeChange target, since ldproxy expects the information of a temporal 
interval to be given via two separate time instants (the interval start and end). 
In the future, a ShapeChange transformation could be added, to convert a 
property with value type TM_Period (and some indicator that the property 
defines the temporal extent of a type with identity) to two properties, 
with value type TM_Instant, and with the tags _defaultIntervalStart_ / _defaultIntervalEnd_.
...
Since v4.0.0, the ldproxy target supports __rule-ldp2-cls-unionLikeDataType__
for the conversion of unions. With this rule, unions are encoded like
data types, with all properties of the union being treated as having
...
Enumerations and code lists are not converted to top-level provider type definitions. Each «enumeration» / «codelist» that a) is not mapped, b) is not marked to not be encoded, and c) belongs to the schemas selected for processing, is converted to a {id}.yml file. By default, that file is placed in the data/values/codelists folder. However, by setting tagged value _ldpCodelistValuePath_ on the «enumeration» / «codelist», a directory path below the codelists folder can be defined, in which the file will be placed. For example, if tag _ldpCodelistValuePath_ has value "abc/def/", then the yml file is placed in folder data/values/codelists/abc/def. Likewise, a file name prefix can be defined: for example, by setting tag _ldpCodelistValuePath_ to "xyz_", the yml file name will have that prefix. The ldproxy target thus also converts enumerations to ldproxy code lists. The ID thereby is the enumeration / code list name, with all non-word characters replaced by '_'.

The following information items are encoded within the YAML file:

* id: the enumeration / code list ID
* label: value as derived using the _labelTemplate_ (see the <<ldproxy2_schemaconversionrules_documentation,documentation>> section for further details)
* description: value as derived using the _descriptionTemplate_ 
(see the <<ldproxy2_schemaconversionrules_documentation,documentation>> 
...
In addition, whenever a property, whose value type is the enumeration / code list, the following information items are added to the property encoding in the provider configuration:
...
[[ldproxy2_schemaconversionrules_properties_propertyid]]
==== Property ID

Default behavior is to encode the property name as the ID of the property in the ldproxy configuration. 

Since ShapeChange v4.1.0, the default can be changed by adding 
`rule-ldp2-all-propertyIdByTaggedValue` to the encoding rule that applies to 
the (main) application schema that is being converted. The conversion
rule requires that target parameter `taggedValueForPropertyId` has a 
non-empty value. The parameter identifies the name of a tagged value.
The tagged value - in lower-case - is used as the ID of the property.
If the tag is blank for a given property, then the default behavior
applies - i.e., the property name is used as ID. The conversion rule
can be useful in situations where the application schema elements
have short code values assigned to them via tagged values, and the
resulting ldproxy configuration shall make use of those codes.
...
tagged value _ldpSourcePaths_ (Note: this mechanism is available since v3.1.0). 
Explicitly configuring source paths via tagged values
...
** the value type is mapped to or implemented by a simple type, and:
*** the map entry has parameter _measure_, <<rule-ldp2-prop-measure>> 
applies to the property, and the tagged value whose name is defined by 
target parameter _uomTaggedValueName_ has no value, then the value type 
is implemented as a measure object, the `type` is OBJECT_ARRAY, and a 
measure object is created in the property definition as defined <<measure_object,below>>.
*** otherwise, the `type` is VALUE_ARRAY, and an additional `valueType` member is added, with the name of the simple type as value.
...
** If the value type is mapped to a simple ldproxy type, and:
*** the map entry has parameter _measure_, <<rule-ldp2-prop-measure>> 
applies to the property, and the tagged value whose name is defined by 
target parameter _uomTaggedValueName_ has no value, then the value type 
is implemented as a measure object, the `type` is OBJECT, and a measure 
object is created in the property definition as defined <<measure_object,below>>.
*** otherwise, `type` is the target type defined by the map entry.
...
https://github.com/ldproxy/ldproxy/issues/1119[ldproxy issue 1119].
...
[[measure_object]]
The definition of a measure object within a property definition is as follows:

* `objectType: Measure`
* `sourcePath`: As defined for the property (for details, see 
section <<ldproxy2_schemaconversionrules_properties_sourcepaths>>)
* `properties`:
** `value`:
*** `label`: value of target parameter `measureValueLabel` (default is: `value`)
*** `sourcePath`: `value`
*** `type`: target type defined by the map entry
*** `constraints required: true`
** `uom`:
*** `label`: value of target parameter `measureUomLabel` (default is: `unit of measure`)
*** `sourcePath`: `uom`
*** `type`: STRING
*** `constraints required: true`
...
[[rule-ldp2-prop-measure]]
(since v4.1.0)
With __rule-ldp2-prop-measure__, the behavior for measure valued properties is extended 
for cased in which the unit tagged value (identified via target parameter _uomTaggedValueName_) 
has no value. In other words, the rule only applies to measure typed properties where the 
unit of measure is not fixed. In such a case, the value type is encoded as a measure object, 
as defined in section <<ldproxy2_schemaconversionrules_properties_propertytype>>.
...
If __rule-ldp2-all-linkObjectAsFeatureRef__ is set in the encoding rule, and 
target parameter `embeddingForFeatureRefs` is set to true, then all properties with a 
feature or object type as value type that have tagged value `inlineOrByReference=inline`
 will be encoded in the ldproxy provider configuration with member `embed` 
 and value ALWAYS.
...
[[ldproxy2_schemaconversionrules_specificrules_geoinfodok]]
==== GeoInfoDok

(since v4.0.0)

__rule-ldp2-all-gid__ supports the creation of ldproxy configurations 
for GeoInfoDok application schemas (which are maintained by the 
German mapping agencies). The data is assumed to be stored within a
relational database.

The conversion requires that SQL encoding 
infos are configured, which provide detailed information about how
relevant model elements are stored within the database.

Encoding of metadata types (from ISO 19115) is restricted to relevant components.
Currently, only a subset of LI_Lineage is supported by the conversion.
Support for additional metadata model elements can be added in future
versions of ShapeChange.


[[ldproxy2_schemaconversionrules_specificrules_storedqueries]]
==== Stored Queries

(since v4.1.0)

Stored query files are created, if an according `LdproxyStoredQueryDefinitions` elements is
placed within the `advancedProcessConfigurations` element of the ldproxy target configuration.

NOTE: This feature is intended to generate rudimentary stored query files, which may
very likely need to be updated manually. However, tedious repetitive work to create
the base query structures for relevant collections of the API is meant to be automated
with this stored query generation feature.

The structure of the `LdproxyStoredQueryDefinitions` element is defined in the 
https://shapechange.net/resources/schema/ShapeChangeConfiguration.xsd[ShapeChange Configuration XML Schema].  It has the `storedQuery` child element (cardinality 1..*), which can be used to
define multiple `LdproxyStoredQuery` elements.

The structure of the `LdproxyStoredQuery` element has the following child elements:

* id (cardinality: 1, type: string): identifier of the stored query, also the name for the resulting file
* title (cardinality: 0..1, type: string, default: _none_): title of the stored query
* description (cardinality: 0..1, type: string, default: _none_): description of the stored query 
* limit (cardinality: 0..1, type: positive integer, default: _none_): maximum number of hits to return
* crs (cardinality: 0..1, type: string): CRS to use when executing the stored query
* queryDefinition (cardinality: 1): Defines the kind of query to be created.
** PropertyEqualToCollectionQuery: The stored query shall retrieve objects based upon an 
equality check of a parameter against a particular feature property.
*** property (cardinality: 1, type: string): The name of the feature property. 
If collection specific properties must be used, manually edit the property 
name after generation, as necessary. 
*** parameter (cardinality: 1, type: string): The name of the stored query parameter. 
An according string-typed parameter will be added to the resulting stored query.

NOTE: A `profiles` section needs to be copied manually to the resulting stored query file(s).
...
[[ldproxy2_uncovered_configuration_items]]
== Uncovered ldproxy configuration items

Ldproxy provides numerous configuration items, 
many of which do not depend on the model itself. 
It is expected that such configuration items are
defined in default and override ldproxy configuration files.
The ShapeChange ldproxy target does not need to
cover the generation of these configuration items.