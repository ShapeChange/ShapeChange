...
_de.interactive_instruments.shapechange.core.target.json.JsonSchemaTarget._
...
* (since v4.0.0) A format for feature identifiers can also be specified - for 
mapped types via map entry characteristic `collectionIdFormat`, for all other 
cases via target parameter <<featureRefIdFormat>>.
...
[[rule-json-all-featureRefs-alwaysSimple]]
==== rule-json-all-featureRefs-alwaysSimple

(since v4.0.0)

Extends the behavior of <<rule-json-all-featureRefs>>, so that
only simple reference schemas are created for profile `rel-as-key`.

This conversion rule was created in support of code generation from JSON Schema.
It can be useful to set parameter <<featureRefWithAnyCollectionId>> to `false`,
or to omit that parameter (since its default value is `false`), because then
`x-ogc-collectionId` will be created, with the applicable collection identifier(s).
...
of an enumeration (except for any enumeration with name "Boolean [ignoring case]), 
with one member per encoded enum. The name of such a member
...
NOTE: Since v4.0.0, it is possible to control whether enum descriptions are also created
for enum valued properties via target parameter <<enumDescriptionOnEnumerationValuedProperties>>.
...
[[rule-json-prop-specificChecksForSupertypeValuedProperties]]
==== rule-json-prop-specificChecksForSupertypeValuedProperties

(since v4.0.0)

For properties that satisfy all of the following conditions:

* the value type may be encoded inline,
* no specific list of value type options are defined for the property
via <<rule-json-cls-valueTypeOptions>>,
* the value type is not mapped to a simple type,
* a JSON Schema definition exists (defined via map entries, or generated
by the target) for the value type, 
* the value type has a non-abstract subtype

a set of restrictions is created, using an if-then-else construct.
The restrictions ensure that based upon the set of non-abstract subtypes
of the value type (including that type, if it is non-abstract, too), 
type specific JSON Schema references are used in the validation. That
enables a form of value type specific checks, which otherwise is
not possible with JSON Schema.

The mechanism depends on the presence of a type identifying member in
property values, and thus _rule-json-prop-specificChecksForSupertypeValuedProperties_ should always
be used in combination
with rule <<rule-json-cls-name-as-entityType>>.
...
Type: Enum (one of "uri", "uri-reference")
...
[[enumDescriptionOnEnumerationValuedProperties]]
==== enumDescriptionOnEnumerationValuedProperties
...
Explanation: If _true_, then enum descriptions are also created for
properties whose value type is an enumeration (unless the name of the
enumeration is "Boolean" [ignoring case]).

Applies to Rule(s):
_xref:./JSON_Schema.adoc#rule-json-cls-documentation-enumDescription[rule-json-cls-documentation-enumDescription]_
...
Explanation: The format of feature identifiers in non-external collection(s).
For example, 'uuid', in case that target parameter <<featureRefIdTypes>> has value 'string'.
...
* collectionIdFormat: The format of the feature identifiers used in the collection(s).
For example, 'uuid' - in case that collectionIdTypes is 'string'.
...
[source,xml,linenumbers]
----------
<Target class="de.interactive_instruments.shapechange.core.target.json.JsonSchemaTarget" mode="enabled">
