...
[[useNameInCaseOfBlankInitialValue]]
==== useNameInCaseOfBlankInitialValue
...
+++Explanation+++: If set to true, then whenever
the initial value of an enum / code has no value,
i.e. the string value is null or empty, the name
of the enum / code is used instead.
