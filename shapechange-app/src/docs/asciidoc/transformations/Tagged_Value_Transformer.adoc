...
[[rule-trf-taggedValue-createClassifierNamespaceTags]]
==== rule-trf-taggedValue-createClassifierNamespaceTags

For each class of the schemas selected for processing,
this rule sets tagged value _namespace_ to the value of
the target namespace of the schema that owns the class. 
If no target namespace is defined for the schema, the 
empty string will be set as tag value.
...
[[rule-trf-taggedValue-createReversePropertyNameTag]]
==== rule-trf-taggedValue-createReversePropertyNameTag

(since v4.1.0)

For each association role of bi-directional associations, where at least one role is
within the schemas selected for processing, a tagged value `reversePropertyName` is added, 
providing the name of the reverse property,i.e. the property at the other end of the 
association.


[[rule-trf-taggedValue-createAssociationRoleSourceOrTargetTag]]
==== rule-trf-taggedValue-createAssociationRoleSourceOrTargetTag

(since v4.1.0)

For each association role of bi-directional associations, where at least one role is
within the schemas selected for processing, a tagged value `sourceOrTarget` is added, 
indicating whether the property fulfills the source or the target role in this relationship.

NOTE: The UML specification does not have a concept of 'source' and 'target' 
for association roles. It does have it for directed relationships such as dependency 
or generalization. However, in some cases - like encoding a relationship between
two classes within a database - it can be useful to differentiate properties of a
bi-directional association, categorizing them as source and target role, respectively.
Enterprise Architect also differentiates association roles - in fact, all connector ends - this way.

[[rule-trf-taggedValue-createAssociationRoleDependentPartTag]]
==== rule-trf-taggedValue-createAssociationRoleDependentPartTag

(since v4.1.0)

Collects navigable role configuration information for the associations
in the schemas selected for processing, as defined by the 
xref:../targets/Coretable.adoc#generateNavigableRolesConfig[Coretable target parameter _generateNavigableRolesConfig_].
For each navigable association role that is part of an ownership relation,
the transformation sets tag `dependentPart` with the information on `dependent_part` from the
navigable role configuration item (either `source` or `target`).

This rule can be useful in combination with additional tools that derive
artifacts for applications that use a coretable database.
