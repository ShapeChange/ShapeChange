...
_de.interactive_instruments.shapechange.core.transformation.flattening.Flattener._
...
parameters _flattenDataTypesExcludeRegex_, _flattenUnionTypesExcludeRegex_, 
_flattenObjectTypes_ and _flattenObjectTypesIncludeRegex_).
...
|*flattenUnionTypesExcludeRegex* |Optional
...
[[rule-trf-all-removePackage]]
==== rule-trf-all-removePackage

(since v4.1.0)

Removes the packages identified by the <<removePackage>> parameter from the
application schema in the model. This includes all subpackages as well
as contained classes. For each class that is removed, all of its inheritance 
relationships are removed as well. However, properties (attributes or 
associations / association roles) whose value type is such a removed class
will be kept (an association role is thereby turned into an attribute).
...
|<<removePackage>> | Required (if the parameter is omitted,
...
https://www.ogc.org/initiatives/ugas-2020/[OGC UML-to-GML
...
all but a certain set of data types from flattening. Same for union types,
with parameter _xref:./Flattener.adoc#flattenUnionTypesExcludeRegex[flattenUnionTypesExcludeRegex].
...
* xref:./Flattener.adoc#flattenUnionTypesExcludeRegex[flattenUnionTypesExcludeRegex]
* flattenObjectTypes
...
This parameter allows to identify the union types that shall NOT
be flattened. The value of this parameter contains a (Java compliant)
regular expression which, if it matches the name of a union type, marks
it to be excluded from type flattening.

NOTE: If you only want to flatten a certain set of union types from the
schemas selected for processing, then you can use 'negative lookahead'
in the regular expression to actually ensure that these types are
flattened, but the other union types are excluded.
...
[[flattenInheritanceKeepCardinalityForAssociationRoleWithAtMostOneNonAbstractTypeOrSubtype]]
==== flattenInheritanceKeepCardinalityForAssociationRoleWithAtMostOneNonAbstractTypeOrSubtype
...
Default Value: false (for backwards-compatibility)

Behavior:

If the value type of an association role is a supertype, the association is
processed by _rule-trf-cls-flatten-inheritance_. The default behavior is to
set the minimum occurrence of such association roles to 0 when creating
value type/subtype specific copies of the association. However, if there is
only one non-abstract type in the set of the value type and its (direct and
indirect) subtypes, changing the cardinality this way is not necessary (since
there is really only one possible value type; all other cases with abstract
value type will be removed) and may not be desired. In order to prevent the
cardinality change in such a situation, set this transformation parameter to
'true'.
...
Behavior: Defines the name of the unit of measure tagged value in
subrule
_rule-trf-prop-flatten-measure-typed-properties-fixed-uom-suffix_ of
xref:./Flattener.adoc#rule-trf-prop-flatten-measure-typed-properties[rule-trf-prop-flatten-measure-typed-properties].
...
Any of the packages (identified by fully qualified name in the model, 
for example "Model root::Package 1::Package 2") listed in
...
Behavior: Defines the separator to be put between the name 
