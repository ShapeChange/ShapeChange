...
Since v4.0.0 of ShapeChange, the switch can be restricted to certain property,
identified via parameter <<propertiesToSwitch>>.
...
Explanation: Identifies the properties for which type switching
shall be applied. Each property identifier is either the property name,
or the property name scoped to a class (using the pattern 
`{class name} + '.' + {property name}`). If the parameter is set, then only 
those properties which match one of the configured identifiers are
potential candidates for type switching.

Applies to Rule(s): <<rule-trf-switchValueTypes>>
