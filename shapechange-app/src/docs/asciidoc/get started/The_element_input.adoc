...
NOTE: This also determines the order in which packages and classes are written by
targets that serialise a model rather than process it type by type, such as
xref:../targets/Model_Export.adoc[_Model Export_]. A target parameter of the same
name takes precedence over this input parameter.
...
_de.interactive_instruments.shapechange.core.model.Transformer_ interface.
...
Input transformers can be configured to modify the input model itself. 

For further details on input transformers, see 
