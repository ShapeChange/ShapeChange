...
in-memory transformations of the input model that was read, 
before creating the target representations on the result of 
...
NOTE: Since v4.1.0, input transformers can be configured, to actually modify
the input model itself. For further details on input transformers, see 
xref:../input transformers/InputTransformers.adoc#InputTransformers[here].

image::../images/processing_with_input_transformations.png[processing with input transformations]

Use of input transformers is optional.

A sample configuration with input transformers can look like this:
...
        <!-- ... -->
        <inputTransformers>
            <InputTransformer class="..." mode="enabled" id="IT_A">
                <!-- ... -->
            </InputTransformer>
            <InputTransformer class="..." mode="enabled" id="IT_B">
                <!-- ... -->
            </InputTransformer>
            <InputTransformer class="..." mode="enabled" id="IT_C">
                <!-- ... -->
            </InputTransformer>
        </inputTransformers>
    </input>
